...
import { X, Cpu, Zap, Layers, TreeDeciduous, Car, Building2, Mountain, LampFloor, Globe, Circle, Hexagon, Square, Eraser, Download, FileJson, Loader2, Moon, Sun, Map as MapIcon, Satellite, Type, Briefcase, Activity, Upload, Save, FolderOpen, PencilRuler, ArrowLeftRight, Grid3X3, AlertTriangle, Droplets } from 'lucide-react';
                ...
                <div className="grid grid-cols-1 gap-2">
                  ...
                  {settings.layers.terrain && (
                    <div className="space-y-2 ml-8">
                      <div className={`flex items-center gap-3 p-2 rounded-lg border transition-all ${settings.layers.slopeAnalysis ? 'bg-slate-800 border-orange-500/50' : 'bg-slate-900 border-slate-800'}`}>
                        <button
                      ...
                      <div className={`flex items-center gap-3 p-2 rounded-lg border transition-all ${settings.layers.hydrology ? 'bg-slate-800 border-blue-500/50' : 'bg-slate-900 border-slate-800'}`}>
                        <button
                          onClick={() => toggleLayer('hydrology')}
                          className="flex items-center gap-2 text-xs w-full text-left"
                        >
                          <Droplets size={14} className={settings.layers.hydrology ? 'text-blue-400' : 'text-slate-600'} />
                          <span className={settings.layers.hydrology ? 'text-blue-200' : 'text-slate-500'}>Mapeamento Hidrológico (Talvegues)</span>
                          <div className={`ml-auto w-2 h-2 rounded-full ${settings.layers.hydrology ? 'bg-blue-500' : 'bg-slate-700'}`} />
                        </button>
                      </div>
                    </div>
                  )}
