...
def test_layer_creation(dxf_gen):
    ...
    assert 'TOPO_EDIFICACAO' in dxf_gen.doc.layers
    assert 'TOPO_VIAS' in dxf_gen.doc.layers
    assert 'TOPO_VEGETACAO' in dxf_gen.doc.layers
...
def test_building_extrusion(dxf_gen):
    ...
    thickness1 = dxf_gen._get_thickness(tags1, 'TOPO_EDIFICACAO')
    assert thickness1 == 15.0

    # Case 2: Levels
    tags2 = {'building': 'yes', 'building:levels': '4'}
    thickness2 = dxf_gen._get_thickness(tags2, 'TOPO_EDIFICACAO')
    assert thickness2 == 12.0 # 4 * 3.0

    # Case 3: Default
    tags3 = {'building': 'yes'}
    thickness3 = dxf_gen._get_thickness(tags3, 'TOPO_EDIFICACAO')
