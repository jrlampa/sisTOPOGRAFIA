...
import { useBtPoleOperations } from "./useBtPoleOperations";
...
export function useBtCrudHandlers({
  ...
  const clearBtExportHistory = () => {
    setAppState(
      { ...appState, btExportSummary: null, btExportHistory: [] },
      true,
    );
  ...
  const validateBtBeforeExport = (): boolean => {
    ...
    if (settings.projectType === "clandestino") {
      ...
      if (!Number.isInteger(area)) {
        showToast(
          "A área clandestina deve ser inteira para casar com a tabela da planilha.",
          "error",
        );
        return false;
      }
      ...
      if (getClandestinoKvaByArea(area) === null) {
        showToast(
          `Área clandestina fora da tabela (${areaRange.min}-${areaRange.max} m²).`,
          "error",
        );
        return false;
      }
    ...
    const edgeWithoutConductors = btTopology.edges.find(
      (edge) =>
        getEdgeChangeFlag(edge) !== "remove" && edge.conductors.length === 0,
    );
    if (edgeWithoutConductors) {
      showToast(
        `Trecho ${edgeWithoutConductors.id} sem condutores definidos.`,
        "error",
      );
      return false;
    }
    ...
    if (settings.projectType === "ramais") {
      ...
      if (transformerWithoutReadings) {
        showToast(
          `Transformador ${transformerWithoutReadings.id} sem leituras.`,
          "error",
        );
        return false;
      }
