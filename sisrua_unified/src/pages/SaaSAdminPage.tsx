    ...
    try {
      ...
      if (rTenants.ok) {
        ...
        setMetricas({
          ...
          totalJobs30d: lista.reduce(
            (acc, t) => acc + (t.jobsUltimos30d ?? 0),
            0,
          ),
...
export default function SaaSAdminPage() {
  ...
  const [token, setToken] = useState(
    () => localStorage.getItem("sisrua_token") ?? "",
  );
    ...
    >
      ...
      {autenticado && (
        <>
            ...
            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
              ...
              <MetricCard
                ...
                value={
                  carregando
                    ? "—"
                    : `${(metricas?.taxaSucesso30d ?? 0).toFixed(1)}%`
                }
                sub="últimos 30 dias"
              />
              <MetricCard
          ...
          {/* ── Rodapé de segurança ── */}
          <p
            className={`mt-8 text-center text-xs ${isDark ? "text-slate-700" : "text-slate-300"}`}
          >
            Acesso restrito · Todas as ações são registradas na trilha de
            auditoria forense.
          </p>
        </>
      )}
