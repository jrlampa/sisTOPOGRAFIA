...
import {
  ...
  ShieldCheck, Sliders, TrendingUp, Users, Zap,
} from "lucide-react";
      ...
      <div className="flex flex-col gap-3">
        ...
        {saude.uptime != null && (
          <div className="flex items-center justify-between">
            <span className={`text-sm ${isDark ? "text-slate-300" : "text-slate-600"}`}>Uptime</span>
...
function TenantRow({ tenant, isDark }: { tenant: TenantResumo; isDark: boolean }) {
  return (
    <tr className={`border-b ${isDark ? "border-white/5 hover:bg-white/5" : "border-slate-100 hover:bg-slate-50"}`}>
      ...
      <td className={`px-4 py-3 text-center font-mono text-xs ${isDark ? "text-slate-400" : "text-slate-500"}`}>
        {tenant.usuarios ?? 0}
      </td>
...
export default function SaaSAdminPage() {
  ...
  const { tenants, saude, metricas, carregando, erro, recarregar } =
    useDadosAdmin(autenticado ? token : "");
  ...
  return (
    <PageShell isDark={isDark} onToggleTheme={() => setIsDark((v) => !v)} role="plataforma">
      ...
      {/* ── Autenticação ── */}
      {!autenticado && (
        <div className={`mb-8 rounded-2xl border p-6 ${isDark ? "border-white/10 bg-white/5" : "border-slate-200 bg-white"}`}>
      ...
      {autenticado && (
        <>
          ...
          <section className="mb-8" aria-labelledby="metricas-title">
            <h2 id="metricas-title" className={`mb-4 text-xs font-semibold uppercase tracking-wider ${isDark ? "text-slate-400" : "text-slate-500"}`}>
              Métricas globais da plataforma
            </h2>
            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
              <MetricCard label="Tenants ativos" cor="sky" icon={Building2}
                value={carregando ? "—" : (metricas?.totalTenants ?? 0)}
                sub="organizações cadastradas" />
              <MetricCard label="Jobs (30 dias)" cor="emerald" icon={Activity}
                value={carregando ? "—" : (metricas?.totalJobs30d ?? 0)}
                sub="exportações processadas" />
              <MetricCard label="Taxa de sucesso" cor="violet" icon={TrendingUp}
                value={carregando ? "—" : `${(metricas?.taxaSucesso30d ?? 0).toFixed(1)}%`}
                sub="últimos 30 dias" />
              <MetricCard label="Alertas ativos" cor="amber" icon={AlertTriangle}
          ...
          <div className="mb-8 grid gap-6 lg:grid-cols-3">
            ...
            <div className="lg:col-span-1">
              {saude ? (
                ...
                <div className={`flex h-full items-center justify-center rounded-2xl border p-6 text-sm ${isDark ? "border-white/10 text-slate-500" : "border-slate-200 text-slate-400"}`}>
            ...
            <div className="lg:col-span-2">
              <div className={`rounded-2xl border p-6 ${isDark ? "border-white/10 bg-white/5" : "border-slate-200 bg-white"}`}>
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  {[
                    { label: "Gerenciar Tenants",       to: "/admin",      icon: Building2, desc: "Criar, ativar, desativar" },
                    { label: "Feature Flags Globais",    to: "/admin",      icon: Sliders,   desc: "Rollout por tenant/região" },
                    { label: "Capacity Planning",        to: "/admin",      icon: BarChart3, desc: "Histórico e metas de jobs" },
                    { label: "Gestão de Vulnerabilidades", to: "/admin",    icon: ShieldCheck, desc: "CVSS SLA por severidade" },
                  ].map((a) => (
                    <Link
                      key={a.label}
                      to={a.to}
                      className={`flex items-center gap-3 rounded-xl border p-3 transition-all hover:border-indigo-500/30 ${
                        isDark ? "border-white/10 hover:bg-white/10" : "border-slate-100 hover:bg-slate-50"
                      }`}
                    >
                      ...
                      <div>
                        <p className={`text-xs font-semibold ${isDark ? "text-slate-200" : "text-slate-800"}`}>{a.label}</p>
                        <p className={`text-xs ${isDark ? "text-slate-500" : "text-slate-400"}`}>{a.desc}</p>
                      </div>
                    </Link>
                  ))}
                </div>
          ...
          <section aria-labelledby="tenants-title">
            ...
            >
              ...
              {tenantsExpanded ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}
            </button>
