...
import {
  ...
  ChevronRight, Clock, FileDown, Map, RefreshCw,
...
function useQuotas(tenantId: string) {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE_URL}/admin/quotas`)
      .then((r) => r.ok ? r.json() as Promise<QuotaStatus[]> : Promise.reject(new Error(`HTTP ${r.status}`)))
      .then(setDados)
      .catch(() => setDados([]))
...
function useSlos(tenantId: string) {
  ...
  useEffect(() => {
    if (!tenantId) return;
    setCarregando(true);
    fetch(`${API_BASE_URL}/admin/servicos?tenantId=${encodeURIComponent(tenantId)}`)
      .then((r) => r.ok ? r.json() as Promise<SloStatus[]> : Promise.reject(new Error(`HTTP ${r.status}`)))
      .then(setDados)
      .catch(() => setDados([]))
      .finally(() => setCarregando(false));
  }, [tenantId]);
...
function KpiCard({
  label, value, sub, icon: Icon, cor,
}: { label: string; value: string | number; sub?: string; icon: React.ElementType; cor: string }) {
...
function SloChip({ status }: { status: SloStatus["status"] }) {
  if (status === "ok") return <span className="flex items-center gap-1 rounded-full bg-emerald-500/15 px-2 py-0.5 text-xs font-semibold text-emerald-400"><CheckCircle2 className="h-3 w-3" />OK</span>;
  if (status === "degradado") return <span className="flex items-center gap-1 rounded-full bg-amber-500/15 px-2 py-0.5 text-xs font-semibold text-amber-400"><AlertTriangle className="h-3 w-3" />Degradado</span>;
  return <span className="flex items-center gap-1 rounded-full bg-rose-500/15 px-2 py-0.5 text-xs font-semibold text-rose-400"><XCircle className="h-3 w-3" />Violado</span>;
}
...
export default function DashboardPage() {
  ...
  const { dados: kpis, carregando: kpiLoad, erro: kpiErro, recarregar } = useKpis(tenantId);
    ...
    <PageShell isDark={isDark} onToggleTheme={() => setIsDark((v) => !v)}>
      ...
      <div className="mb-8 flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className={`mt-1 text-sm ${isDark ? "text-slate-400" : "text-slate-500"}`}>
            Métricas de operação, status de SLO e consumo de quotas do seu tenant.
          </p>
        </div>
      ...
      {semTenant && (
        <div className="mb-8 rounded-xl border border-amber-500/30 bg-amber-500/10 p-4 text-sm text-amber-400">
          <strong>Tenant não configurado.</strong> Configure seu <code className="font-mono">sisrua_tenant_id</code> no{" "}
          <Link to="/admin" className="underline">painel administrativo</Link> para visualizar as métricas.
        </div>
      )}

      {/* ── KPIs ── */}
      <section className="mb-8" aria-labelledby="kpi-title">
        <h2 id="kpi-title" className={`mb-4 text-xs font-semibold uppercase tracking-wider ${isDark ? "text-slate-400" : "text-slate-500"}`}>
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          <KpiCard
            label="Jobs realizados" cor="sky"
            value={kpiLoad ? "—" : (kpis?.jobsUltimas24h ?? 0)}
            sub="últimas 24h" icon={Activity}
          />
          <KpiCard
            label="Taxa de sucesso" cor="emerald"
            value={kpiLoad ? "—" : `${(kpis?.taxaSucessoPct ?? 0).toFixed(1)}%`}
            sub={`${kpis?.sucesso ?? 0} sucesso · ${kpis?.falha ?? 0} falha`} icon={TrendingUp}
          />
          <KpiCard
            label="Tempo médio" cor="violet"
            value={kpiLoad ? "—" : `${((kpis?.tempoMedioMs ?? 0) / 1000).toFixed(1)}s`}
            sub="por job de exportação" icon={Clock}
          />
          <KpiCard
            label="Total processado" cor="amber"
            value={kpiLoad ? "—" : (kpis?.total ?? 0)}
            sub="desde o início do mês" icon={FileDown}
          />
        </div>
      </section>
      ...
      <section className="mb-8" aria-labelledby="slo-title">
        <h2 id="slo-title" className={`mb-4 text-xs font-semibold uppercase tracking-wider ${isDark ? "text-slate-400" : "text-slate-500"}`}>
          Status de SLO por Serviço
        </h2>
        {sloLoad ? (
          <div className={`rounded-xl border p-6 text-center text-sm ${isDark ? "border-white/10 text-slate-500" : "border-slate-200 text-slate-400"}`}>
            Carregando dados de SLO…
          </div>
        ) : slos.length === 0 ? (
          <div className={`rounded-xl border p-6 text-center text-sm ${isDark ? "border-white/10 text-slate-500" : "border-slate-200 text-slate-400"}`}>
            Nenhum perfil de serviço configurado.{" "}
            <Link to="/admin" className="text-indigo-400 underline">Configurar no Admin</Link>
          </div>
        ) : (
          <div className={`overflow-hidden rounded-2xl border ${isDark ? "border-white/10" : "border-slate-200"}`}>
            <table className="w-full text-sm">
              <thead>
                <tr className={`text-xs font-semibold uppercase tracking-wider ${isDark ? "border-b border-white/10 bg-white/5 text-slate-400" : "border-b border-slate-100 bg-slate-50 text-slate-500"}`}>
              ...
              <tbody>
                {slos.map((s, i) => (
                  ...
                  >
                    ...
                    <td className={`px-4 py-3 text-right font-mono text-xs ${isDark ? "text-slate-400" : "text-slate-500"}`}>
      ...
      <section className="mb-8" aria-labelledby="quota-title">
        <h2 id="quota-title" className={`mb-4 text-xs font-semibold uppercase tracking-wider ${isDark ? "text-slate-400" : "text-slate-500"}`}>
          Consumo de Quotas
        </h2>
        {quotaLoad ? (
          <div className={`rounded-xl border p-6 text-center text-sm ${isDark ? "border-white/10 text-slate-500" : "border-slate-200 text-slate-400"}`}>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {quotas.map((q) => {
              const pct = q.limite > 0 ? Math.min(100, (q.consumido / q.limite) * 100) : 0;
              const barCor = pct >= 90 ? "bg-rose-500" : pct >= 70 ? "bg-amber-500" : "bg-emerald-500";
              return (
                <div key={q.recurso} className={`rounded-2xl border p-5 ${isDark ? "border-white/10 bg-white/5" : "border-slate-200 bg-white"}`}>
                  <div className="mb-2 flex items-center justify-between text-sm">
                    <span className={`font-semibold ${isDark ? "text-slate-200" : "text-slate-800"}`}>{q.recurso}</span>
                    <span className={`font-mono text-xs ${isDark ? "text-slate-400" : "text-slate-500"}`}>
                      {q.consumido}/{q.limite} {q.unidade}
                    </span>
                  </div>
                  <div className={`h-2 w-full overflow-hidden rounded-full ${isDark ? "bg-white/10" : "bg-slate-100"}`}>
                    <div className={`h-full rounded-full transition-all ${barCor}`} style={{ width: `${pct}%` }} />
                  </div>
                  <p className={`mt-1 text-right text-xs ${isDark ? "text-slate-500" : "text-slate-400"}`}>{pct.toFixed(0)}% utilizado</p>
                </div>
              );
            })}
          </div>
      ...
      <section aria-labelledby="quick-title">
        <h2 id="quick-title" className={`mb-4 text-xs font-semibold uppercase tracking-wider ${isDark ? "text-slate-400" : "text-slate-500"}`}>
          Acesso rápido
        </h2>
        <div className="grid gap-4 sm:grid-cols-3">
          {[
            { label: "Novo Projeto",      to: "/app",     icon: Map,      desc: "Extrair área, gerar DXF 2.5D" },
            { label: "Painel Admin",      to: "/admin",   icon: Zap,      desc: "Usuários, quotas, flags, SLA" },
            { label: "Central de Ajuda",  to: "/ajuda",   icon: BarChart3, desc: "Documentação, FAQ e suporte" },
          ].map((item) => (
            <Link
              key={item.to}
              to={item.to}
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className={`text-sm font-semibold ${isDark ? "text-slate-200" : "text-slate-800"}`}>{item.label}</p>
                  <p className={`text-xs ${isDark ? "text-slate-500" : "text-slate-400"}`}>{item.desc}</p>
                </div>
              </div>
              <ChevronRight className={`h-4 w-4 shrink-0 ${isDark ? "text-slate-500" : "text-slate-300"}`} />
            </Link>
          ))}
        </div>
      </section>
