...
export const ProjectPage: React.FC = () => {
  ...
  const handleCreateProject = async (data: Partial<ProjectMetadata>) => {
    const projectId = await ProjectService.createProject(data, INITIAL_APP_STATE);
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm font-bold uppercase tracking-[0.2em] mt-1">
            Gerencie seus recortes e jurisdições espaciais
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-20 bg-white/5 rounded-3xl border border-dashed border-white/10">
          ...
          <p className="text-slate-500 font-bold uppercase tracking-widest text-xs">
            Nenhum recorte encontrado nesta jurisdição.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {filteredProjects.map(project => (
            ...
            >
              {/* Project Cover */}
              <div className="h-32 bg-slate-800 relative overflow-hidden bg-[url('/map-placeholder.svg')] bg-cover">
                <div className="absolute inset-0 bg-gradient-to-t from-slate-900 to-transparent" />
                <div className="absolute top-4 left-4 flex gap-2">
                  <div
                    className={`px-2 py-1 rounded-lg border text-[8px] font-black uppercase tracking-widest ${getStatusStyle(project.status)}`}
                  >
                    {project.status}
                  </div>
              ...
              <div className="p-6 space-y-4">
                <div>
                  <h3 className="text-lg font-black text-white group-hover:text-indigo-400 transition-colors uppercase tracking-tight leading-tight">
                    {project.name}
                  </h3>
