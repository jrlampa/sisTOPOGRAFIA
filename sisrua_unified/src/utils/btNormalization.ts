...
export type BtTransformerChangeFlag = NonNullable<
...
export const getTransformerChangeFlag = (
...
export const normalizeBtEdge = (edge: BtEdge): BtEdge => {
  ...
  const mustHaveConductor =
    edgeChangeFlag === "replace" || edgeChangeFlag === "new";
  const hasConductors = edge.conductors.length > 0;
  const replacementFromConductors = Array.isArray(
...
export const normalizeBtEdges = (edges: BtEdge[]): BtEdge[] =>
  edges.map(normalizeBtEdge);
...
export const normalizeBtPoles = (poles: BtPoleNode[]): BtPoleNode[] =>
  poles.map(normalizeBtPole);

export const normalizeBtTransformer = (
  transformer: BtTransformer,
): BtTransformer => ({
  ...transformer,
...
export const normalizeBtTransformers = (
  transformers: BtTransformer[],
): BtTransformer[] => transformers.map(normalizeBtTransformer);
...
): number => haversineDistanceMeters(from, to);
