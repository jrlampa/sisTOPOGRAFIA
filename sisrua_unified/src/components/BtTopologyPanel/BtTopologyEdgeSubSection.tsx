...
const MEC_DATA: Record<string, { traction: number; weight: number }> = {
    ...
    "185 Al - MX": { traction: 520, weight: 2.20 },
    "MT-4-AWG-AL": { traction: 150, weight: 0.15 },
    "MT-70-PROT": { traction: 250, weight: 0.90 }
};
...
const BtTopologyEdgeSubSection: React.FC<BtTopologyEdgeSubSectionProps> = ({
  ...
  const allCircuits = [
      ...(selectedEdge.conductors || []).map(c => ({ ...c, type: 'BT' as const })),
      ...(selectedEdge.mtConductors || []).map(c => ({ ...c, type: 'MT' as const }))
  ];

  const totalTraction = allCircuits.reduce((acc, c) => acc + (MEC_DATA[c.conductorName]?.traction || 200) * c.quantity, 0);
  const totalWeight = allCircuits.reduce((acc, c) => acc + (MEC_DATA[c.conductorName]?.weight || 0.85) * c.quantity, 0);
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-2xl p-3 border border-slate-200 shadow-sm">
          ...
          <div className="flex gap-1">
            {(["existing", "new", "replace", "remove"] as const).map((flag) => (
                ...
                >
                  {flag === "existing" ? "Mant." : flag === "new" ? "Inst." : flag === "replace" ? "Subst." : "Sucata"}
                </button>
            ))}
          </div>
      </div>
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between px-1">
            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Circuitos no Vão</span>
            <div className="flex gap-1">
                <button 
                    onClick={() => updateEdgeConductors(selectedEdge.id, [...selectedEdge.conductors, { id: nextId("C"), quantity: 1, conductorName: CONDUCTOR_NAMES[0] }])}
                ...
                <button 
                    onClick={() => updateEdgeMtConductors?.(selectedEdge.id, [...(selectedEdge.mtConductors || []), { id: nextId("MT"), quantity: 1, conductorName: MT_CONDUCTOR_NAMES[0] }])}
                ...
                {allCircuits.map((c) => (
                    <motion.div 
                        ...
                        className={`group flex items-center gap-2 p-2 rounded-xl border transition-all ${
                            c.type === 'MT' ? "bg-amber-50/30 border-amber-100" : "bg-white border-slate-100"
                        }`}
                    >
                        <div className={`px-1.5 py-0.5 rounded text-[8px] font-black shrink-0 ${
                            c.type === 'MT' ? "bg-amber-100 text-amber-700" : "bg-blue-100 text-blue-700"
                        }`}>
                            {c.type}
                        </div>
                        ...
                        <div className="flex-1 overflow-hidden">
                            ...
                            <div className="flex items-center gap-3 mt-0.5 opacity-60">
                                <span className="flex items-center gap-0.5 text-[8px] font-bold">
                                    <Activity size={8} /> {(MEC_DATA[c.conductorName]?.traction || 0) * c.quantity} daN
                                </span>
                                <span className="flex items-center gap-0.5 text-[8px] font-bold">
                                    <Weight size={8} /> {(MEC_DATA[c.conductorName]?.weight || 0).toFixed(2)} daN/m
                                </span>
                            </div>
                        </div>
                        ...
                        <button
                            onClick={() => {
                                if (c.type === 'BT') {
                                    updateEdgeConductors(selectedEdge.id, selectedEdge.conductors.filter(item => item.id !== c.id));
                                } else {
                                    updateEdgeMtConductors?.(selectedEdge.id, (selectedEdge.mtConductors || []).filter(item => item.id !== c.id));
                                }
                            }}
      ...
      <div className="bg-slate-900 rounded-2xl p-3 text-white shadow-lg overflow-hidden relative">
          ...
          <div className="relative z-10">
              <div className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-2">Resumo de Esforço do Vão</div>
              <div className="grid grid-cols-2 gap-4">
                  <div>
                      <div className="text-[10px] font-bold opacity-60 uppercase">Tração Total</div>
                      <div className="text-sm font-black text-blue-400">{totalTraction} <small className="text-[10px] opacity-70">daN</small></div>
                  </div>
                  <div>
                      <div className="text-[10px] font-bold opacity-60 uppercase">Peso Estimado</div>
                      <div className="text-sm font-black text-emerald-400">{totalWeight.toFixed(2)} <small className="text-[10px] opacity-70">daN/m</small></div>
                  </div>
              </div>
          </div>
      </div>
