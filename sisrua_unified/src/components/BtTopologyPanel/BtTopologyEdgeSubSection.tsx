  ...
  return (
    <div className="space-y-2 rounded-lg border border-slate-300 bg-slate-50 p-3">
      ...
      {btTopology.edges.length === 0 ? (
        <div className="text-[10px] text-slate-500 italic">Nenhum trecho inserido.</div>
      ) : (
        <React.Fragment>
          <select
          ...
          >
            {btTopology.edges.map(e => (
              <option key={e.id} value={e.id}>{e.id} ({e.fromPoleId} ↔ {e.toPoleId})</option>
            ))}
          </select>
          ...
          {selectedEdge && (
            <div className="space-y-3">
              ...
              <div className="space-y-2">
                <div className="text-[10px] font-bold text-slate-500 uppercase">Composição:</div>
                {selectedEdge.conductors.map((c) => (
                  <div key={c.id} className="flex gap-2 items-center">
                    <span className="text-[10px] w-6">{c.quantity}x</span>
                    <span className="text-[10px] flex-1 truncate">{c.conductorName}</span>
                    <button 
                      onClick={() => updateEdgeConductors(selectedEdge.id, selectedEdge.conductors.filter(item => item.id !== c.id))}
