...
interface BtTransformerEdgeSectionProps {
  ...
  updateTransformerReadings: (transformerId: string, readings: BtTransformerReading[]) => void;
  updateTransformerProjectPower: (transformerId: string, projectPowerKva: number) => void;
    ...
    <>
      <div className="space-y-2 rounded-lg border border-slate-300 bg-slate-50 p-3">
        <div className="text-[10px] font-semibold uppercase tracking-wide text-slate-400">
          Transformador ({btNetworkScenario === "asis" ? "leituras da rede atual" : "base de projeto"})
        </div>
          ...
          <>
            ...
            {selectedTransformer && onBtSetTransformerChangeFlag && (
              <div className="flex flex-wrap gap-2 rounded border border-slate-200 bg-slate-100/70 p-2">
                <button
                  onClick={() =>
                    onBtSetTransformerChangeFlag(selectedTransformer.id, "remove")
                  }
                ...
                <button
                  onClick={() =>
                    onBtSetTransformerChangeFlag(selectedTransformer.id, "replace")
                  }
                ...
                <button
                  onClick={() =>
                    onBtSetTransformerChangeFlag(selectedTransformer.id, "existing")
                  }
                ...
                {(() => {
                  ...
                  return (
                    <>
                      <div className="rounded border border-slate-200 bg-white p-2">
                        <div className="grid grid-cols-4 gap-2">
                              ...
                              const inferredCurrent =
                                Math.round(
                                  (nextCorrectedDemandKva /
                                    (CURRENT_TO_DEMAND_CONVERSION * temperatureBase)) *
                                    100,
                                ) / 100;
                              updateTransformerReadings(selectedTransformer.id, [
                                {
                          ...
                          <NumericTextInput
                            ...
                            onChange={(next) =>
                              updateTransformerProjectPower(selectedTransformer.id, next)
                            }
                      ...
                      <button
                        onClick={() => {
                          const estimatedDemandKw = transformerDebug?.estimatedDemandKw;
                          const fallbackDemandKw = selectedTransformer.demandKw ?? 0;
                      ...
                      <div className="rounded border border-slate-300 bg-white p-2 text-[10px] text-slate-700 space-y-1">
                        ...
                        <div>
                          Demanda maxima: {formatBr(hasReadings ? demandMaxKw : effectiveDemandKw)}
                          {" "}kVA
                        </div>
                        <div>
                          Carregamento atual:{" "}
                          {loadingPct === null ? "#DIV/0!" : `${loadingPct.toFixed(2)}%`}
                        </div>
                      ...
                      {transformerDebug && (
                        <div className="rounded border border-indigo-300 bg-indigo-50 p-2 text-[10px] text-indigo-900 space-y-1">
                          ...
                          <div>
                            Demanda estimada automatica: {formatBr(transformerDebug.estimatedDemandKw)}
                            {" "}kVA
                          </div>
                          <div>
                            Fonte: particao eletrica da rede considerando seccionamentos BT.
                          </div>
                        </div>
                      )}
                    </>
          ...
          <>
            ...
            >
              {btTopology.edges.map((edge) => {
                const fromTitle =
                  btTopology.poles.find((pole) => pole.id === edge.fromPoleId)?.title ??
                  edge.fromPoleId;
                const toTitle =
                  btTopology.poles.find((pole) => pole.id === edge.toPoleId)?.title ??
                  edge.toPoleId;
                return (
            ...
            {selectedEdge && (
              <div className="space-y-2">
                {(() => {
                  const selectedEdgeFlag: BtEdgeChangeFlag = getEdgeChangeFlag(selectedEdge);
                  return (
                    <div className="rounded border border-slate-200 bg-slate-100/70 p-2">
                      ...
                      <div className="mt-1 text-[10px] text-slate-600">
                        ...
                        <span className="font-semibold text-fuchsia-700">Existente (Magenta)</span>
                      </div>
                ...
                {getEdgeChangeFlag(selectedEdge) === "replace" && (
                  <div className="rounded border border-cyan-300 bg-cyan-50 p-2 text-[10px] text-cyan-900">
                    ...
                    <div className="mt-1">
                      Este bloco define o novo condutor que ficará no trecho após a substituição.
                    </div>
                  </div>
                )}
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        updateEdgeConductors(
                          selectedEdge.id,
                          selectedEdge.conductors.map((item) =>
                            item.id === entry.id ? { ...item, conductorName } : item,
                          ),
                        );
                      }}
                      ...
                      >
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            updateEdgeReplacementFromConductors(
                              selectedEdge.id,
                              (selectedEdge.replacementFromConductors ?? []).map((item) =>
                                item.id === entry.id ? { ...item, quantity } : item,
                              ),
                            );
                          }}
                        ...
                        <button
                          onClick={() => {
                            updateEdgeReplacementFromConductors(
                              selectedEdge.id,
                              (selectedEdge.replacementFromConductors ?? []).filter(
                ...
                <div className="rounded border border-slate-300 bg-white p-2 text-[10px] text-slate-700">
                  ...
                  <div className="mt-1 grid grid-cols-[auto_120px] items-center gap-2">
                    ...
                    <input
                      ...
                      min={0}
                      step={0.01}
                      value={Number(selectedEdge.cqtLengthMeters ?? selectedEdge.lengthMeters ?? 0)}
                      onChange={(e) => {
                        updateEdgeCqtLengthMeters(
                          selectedEdge.id,
