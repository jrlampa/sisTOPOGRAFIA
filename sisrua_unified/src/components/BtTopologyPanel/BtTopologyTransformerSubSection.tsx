  ...
  return (
    <div className="space-y-2 rounded-xl border border-slate-200 bg-slate-50/50 p-3 dark:bg-zinc-950/20 dark:border-white/5">
      ...
      {btTopology.transformers.length === 0 ? (
        <div className="text-xs text-slate-400 italic py-2">{t.noTransformer}</div>
      ) : (
        <React.Fragment>
          <div className="relative">
            ...
            <button
              ...
              title={t.placeholderTransformerName}
              aria-label={t.placeholderTransformerName}
            >
              <ChevronDown size={14} />
            </button>
            {isTransformerDropdownOpen && (
          ...
          {selectedTransformer && (
            <div className="space-y-4 animate-in fade-in zoom-in-95 duration-200">
              <div className="flex gap-2">
                <button
                  onClick={() =>
                    updateTransformerVerified(selectedTransformer.id, !selectedTransformer.verified)
                  }
                ...
                >
                  {selectedTransformer.verified ? t.btnMarkUnverified : t.btnMarkVerified}
                </button>
              </div>
