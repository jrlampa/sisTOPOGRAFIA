            ...
            {activeTab === "engineering" && (
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div
                    className={`rounded-3xl p-4 border border-transparent ${cqtClass}`}
                  >
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between items-center text-sm">
                      ...
                      <span className="font-bold">
                        {accumulatedData?.localTrechoDemandKva.toFixed(2) ??
                          "0.00"}{" "}
                        kVA
                      </span>
                    </div>
                    <div className="flex justify-between items-center text-sm">
                      ...
                      <span className="font-bold">
                        {accumulatedData?.accumulatedDemandKva.toFixed(2) ??
                          "0.00"}{" "}
                        kVA
                      </span>
                    </div>
                    <div className="flex justify-between items-center text-sm">
                      ...
                      <span className="font-bold">
                        {accumulatedData?.accumulatedClients ?? 0} unidades
                      </span>
                    </div>
                  </div>
                </div>
                ...
                {transformer && (
                  <div className="rounded-3xl bg-amber-50 border border-amber-100 p-5 dark:bg-amber-900/10 dark:border-amber-900/20">
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <div>
                        <div className="text-xs font-black text-amber-900 dark:text-amber-200 uppercase">
                          Transformador Ativo
                        </div>
                        <div className="text-[10px] font-bold text-amber-700/60 font-mono">
                          {transformer.id}
                        </div>
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-4 text-sm font-bold text-amber-900 dark:text-amber-200">
                      ...
                      <div>
                        Leitura: {transformer.demandKva?.toFixed(1) ?? "-"} kVA
                      </div>
                    </div>
                  </div>
                )}
              ...
              >
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {[1, 2, 3, 4].map((i) => {
                      const key = `si${i}` as keyof NonNullable<
                        typeof pole.btStructures
                      >;
                      const val = pole.btStructures?.[key];
                      return (
                        <div
                          key={i}
                          className="p-3 rounded-2xl border border-slate-100 bg-white shadow-sm dark:bg-zinc-800 dark:border-white/5 text-center"
                        >
            ...
            <div className="flex gap-3">
              <button
                onClick={() =>
                  onSetPoleChangeFlag?.(
                    pole.id,
                    pole.verified ? "existing" : "replace",
                  )
                }
