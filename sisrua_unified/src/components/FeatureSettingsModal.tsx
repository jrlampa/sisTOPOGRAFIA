...
export const FeatureSettingsModal: React.FC<FeatureSettingsModalProps> = ({ isOpen, onClose }) => {
  ...
  const userRole =
    user?.email?.includes('admin') || user?.email?.includes('techlead') ? 'tech_lead' : 'editor';
  ...
  const getRobustnessScore = () => {
    ...
    const active = Object.entries(flags).filter(
      ([key, val]) => val && FEATURE_LABELS[key as keyof FeatureFlags]
    ).length;
    return Math.round((active / total) * 100);
  };

  const getSavings = () => {
    const highActive = Object.entries(FEATURE_LABELS).filter(
      ([key, info]) => info.performanceImpact === 'high' && flags[key as keyof FeatureFlags]
    ).length;
    const highTotal = Object.values(FEATURE_LABELS).filter(
      i => i.performanceImpact === 'high'
    ).length;
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/80 backdrop-blur-sm animate-in fade-in duration-300">
      <div className="w-full max-w-2xl bg-slate-900 border border-white/10 rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[85vh]">
        ...
        <div className="p-6 border-b border-white/5 flex items-center justify-between bg-gradient-to-r from-slate-900 to-slate-800">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[10px] text-fuchsia-400/80 font-black uppercase tracking-[0.3em]">
                Painel de Controle de Robustez
              </p>
            </div>
          </div>
          <button
        ...
        {/* Content */}
        <div className="flex-1 overflow-y-auto p-8 space-y-10 custom-scrollbar bg-[url('/grid-subtle.svg')] bg-repeat">
          ...
          <div className="grid grid-cols-3 gap-4 animate-in zoom-in-95 duration-500">
            <div className="p-4 rounded-2xl bg-white/5 border border-white/10 flex flex-col items-center justify-center text-center shadow-lg">
              <div className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1 italic">
                Robustez
              </div>
            ...
            <div className="p-4 rounded-2xl bg-white/5 border border-white/10 flex flex-col items-center justify-center text-center shadow-lg">
              ...
              <div className="text-2xl font-black text-emerald-400">
                +{100 - getRobustnessScore()}%
              </div>
              <div className="text-[8px] font-bold text-emerald-500/60 uppercase mt-1">
                Otimizado
              </div>
            </div>
            <div className="p-4 rounded-2xl bg-white/5 border border-white/10 flex flex-col items-center justify-center text-center shadow-lg">
            ...
            <div className="grid grid-cols-2 gap-3">
              {(Object.entries(PRESETS) as [FeaturePreset, any][]).map(([key, preset]) => (
                ...
                >
                  ...
                  <span className="text-[9px] font-bold text-slate-500 uppercase mt-1 tracking-tighter">
                    Padrão do Sistema
                  </span>
                </button>
              ))}
              ...
              {customPresets.map(preset => (
                <div key={preset.id} className="relative group">
                  ...
                  >
                    ...
                    <span className="text-[9px] font-bold text-indigo-500/60 uppercase mt-1 tracking-tighter">
                      Preset do Usuário
                    </span>
                  </button>
                  <button
                ...
                {Object.entries(FEATURE_LABELS)
                  ...
                  .map(([key, info]) => {
                    ...
                    return (
                      ...
                      >
                        {isEnabled && (
                          <div className="absolute inset-0 bg-gradient-to-r from-fuchsia-500/5 to-transparent rounded-2xl pointer-events-none" />
                        )}

                        <div className="flex gap-5 items-start relative z-10">
                          <div
                            className={`mt-1.5 p-2.5 rounded-xl border transition-colors ${isEnabled ? 'bg-fuchsia-500/10 border-fuchsia-500/20' : 'bg-slate-800 border-transparent'}`}
                          >
                            <Power
                              className={`w-4 h-4 ${isEnabled ? 'text-fuchsia-400' : 'text-slate-600'}`}
                            />
                          </div>
                          <div>
                            <div className="flex items-center gap-2 mb-1">
                              ...
                              <div
                                className={`flex items-center gap-1 px-1.5 py-0.5 rounded text-[8px] font-black uppercase tracking-tighter ${getImpactColor(info.performanceImpact)}`}
                              >
                              ...
                              {isEnabled && health && (
                                <div
                                  className={`flex items-center gap-1 px-1.5 py-0.5 rounded bg-slate-800 border border-white/5 text-[8px] font-black uppercase tracking-tighter ${health.status === 'online' ? 'text-emerald-400' : health.status === 'degraded' ? 'text-amber-400' : 'text-rose-500'}`}
                                >
