...
export default function AdminPage() {
  ...
  const SECOES: SecaoConfig[] = [
    { id: "saude",        titulo: t.sections.saude,        descricao: "Status do painel e conectividade com banco",               icone: Activity,   cor: "emerald" },
    { id: "dashboard",    titulo: t.sections.dashboard,    descricao: "Performance de rede, auditoria e catálogo (via MVs)",      icone: BarChart3,  cor: "fuchsia" },
    { id: "usuarios",     titulo: t.sections.usuarios,     descricao: "Gestão de usuários e atribuição de papéis (RBAC)",         icone: Users,      cor: "blue"    },
    { id: "papeis",       titulo: t.sections.papeis,       descricao: "Estatísticas de distribuição de papéis no sistema",        icone: ShieldCheck, cor: "indigo" },
    { id: "tenants",      titulo: t.sections.tenants,      descricao: "Clientes corporativos ativos na plataforma",               icone: Building2,  cor: "violet"  },
    { id: "quotas",       titulo: t.sections.quotas,       descricao: "Limites de uso configurados por tenant",                   icone: Sliders,    cor: "amber"   },
    { id: "flags",        titulo: t.sections.flags,        descricao: "Configurações de funcionalidades por tenant",              icone: BarChart3,  cor: "orange"  },
    { id: "kpis",         titulo: t.sections.kpis,         descricao: "Observabilidade de negócio: taxa de sucesso e gargalos",   icone: Activity,   cor: "rose"    },
    { id: "servicos",     titulo: t.sections.servicos,     descricao: "Perfis de serviço (SLA/SLO)",                              icone: Network,   cor: "sky"     },
    { id: "retencao",     titulo: t.sections.retencao,     descricao: "Políticas de ciclo de vida e arquivamento por recurso",    icone: Sliders,    cor: "teal"    },
    { id: "capacidade",   titulo: t.sections.capacidade,   descricao: "Histórico de capacidade e metas de jobs simultâneos",      icone: BarChart3,  cor: "cyan"    },
    { id: "vulns",        titulo: t.sections.vulns,        descricao: "Gestão de vulnerabilidades com prazos por severidade",     icone: ShieldCheck, cor: "red"   },
    { id: "classificacao",titulo: t.sections.classificacao,descricao: "Níveis de sensibilidade e políticas de segregação",        icone: Lock,       cor: "purple"  },
    { id: "holdings",     titulo: t.sections.holdings,     descricao: "Grupos empresariais e auditoria cruzada",                  icone: Building2,  cor: "stone"   },
    { id: "finops",       titulo: t.sections.finops,       descricao: "Consumo de APIs e processamento",                          icone: Activity,   cor: "lime"    },
  ];
    ...
    <div className="min-h-screen bg-slate-100 dark:bg-zinc-950 p-4 md:p-8 font-sans">
      ...
      <div className="max-w-4xl mx-auto mb-6">
        <div className="flex items-center gap-3 mb-1">
          ...
          <div>
            <h1 className="text-xl font-black text-slate-900 dark:text-slate-100 tracking-tight">{t.title}</h1>
            <p className="text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400 opacity-70">sisRUA Unified — Autoatendimento Corporativo</p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto space-y-4">
        ...
        {!autenticado && (
          <div className="glass-panel rounded-2xl border-2 border-indigo-700/30 dark:border-indigo-500/40 p-6 shadow-xl backdrop-blur-md">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="font-black text-sm text-indigo-900 dark:text-indigo-200 uppercase tracking-wider">Autenticação Requerida</h2>
            </div>
            <p className="text-xs text-slate-600 dark:text-slate-400 mb-5 leading-relaxed">
              Informe o token de administração para acessar os recursos protegidos do painel central.
            </p>
            <form onSubmit={handleLogin} className="flex flex-col gap-3">
              <div className="flex gap-2">
                ...
                />
                <button type="submit" className="rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white px-6 py-3 text-sm font-black uppercase tracking-widest shadow-lg shadow-indigo-600/25 transition-all active:scale-[0.98]">
                  {t.auth.button}
                </button>
              </div>
            </form>
            {erroAuth && <p className="mt-3 text-xs font-bold text-rose-600 dark:text-rose-400 flex items-center gap-2" role="alert"><AlertCircle size={14} /> {erroAuth}</p>}
          </div>
        )}
        ...
        {autenticado && (
          <div className="glass-panel rounded-2xl border-2 border-amber-700/30 dark:border-amber-500/40 p-4 shadow-lg backdrop-blur-md">
            <label htmlFor="tenant-id-input" className="block text-[10px] font-black uppercase tracking-[0.2em] text-amber-900 dark:text-amber-200 mb-2 opacity-80">
            ...
            <form onSubmit={handleDefinirTenant} className="flex gap-2">
              ...
              />
              <button type="submit" className="rounded-xl bg-amber-600 hover:bg-amber-500 text-white px-5 py-2.5 text-xs font-black uppercase tracking-widest transition-all">
                Definir
              </button>
            </form>
            {tenantIdAtivo && (
              <p className="mt-2 text-[10px] font-bold text-slate-500 dark:text-slate-400 flex items-center gap-1.5" aria-live="polite">
                <span className="h-1.5 w-1.5 rounded-full bg-amber-500 animate-pulse" />
                Filtro ativo: <span className="font-black text-amber-700 dark:text-amber-300">{tenantIdAtivo}</span>
              </p>
            )}
          </div>
        )}
