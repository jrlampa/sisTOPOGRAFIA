...
const getMtEdgeVisualConfig = (edge: MapMtEdge) => {
  ...
  if (flag === "new") return { color: "#ea580c", dashArray: "8 6", weight: 3.5 };
  if (flag === "remove") return { color: "#ef4444", dashArray: "8 6", weight: 3.5 };
  if (flag === "replace") return { color: "#fbbf24", dashArray: undefined, weight: 3.5 };
  return { color: "#d97706", dashArray: undefined, weight: 3.5 };
};

// ─── Componente de Vão MT Individual (Memoizado) ───────────────────────────

interface MtEdgeComponentProps {
  edge: MapMtEdge;
  from: MapMtPole;
  to: MapMtPole;
  popupEdge: MapMtEdge;
...
const MapMtEdgeComponent = React.memo(({
  edge,
  from,
  to,
  popupEdge,
  ...
  popupEventHandlers,
}: MtEdgeComponentProps) => {
  const t = getBtTopologyPanelText(locale).poleVerification;
  const edgeVisual = React.useMemo(() => getMtEdgeVisualConfig(edge), [edge]);
  ...
  return (
    <>
      <Polyline
        ...
        pathOptions={{ color: "#000000", weight: EDGE_HIT_AREA_WEIGHT, opacity: 0.01 }}
      >
        <Popup eventHandlers={popupEventHandlers}>
          <div className="text-xs" onMouseDown={(e) => e.stopPropagation()} onClick={(e) => e.stopPropagation()}>
            <strong>Vão MT: {edge.id}</strong>
            <div className="mt-1 text-slate-700">{popupFrom.title} {"<->"} {popupTo.title}</div>
            <div className="mt-1 text-slate-700">Comprimento: {popupEdge.lengthMeters} m</div>
            <div className="mt-1.5 flex flex-col gap-1.5">
              <div className={POPUP_FLAG_GRID_CLASS}>
                {(["existing", "new", "replace", "remove"] as const).map((flag) => (
                  <button
                    key={flag}
                    onClick={() => onMtSetEdgeChangeFlag?.(edge.id, flag)}
                    className={getFlagButtonClass(edgeChangeFlag === flag, flag)}
                  >
      ...
      <Polyline
        positions={[[from.lat, from.lng], [to.lat, to.lng]]}
        pathOptions={{ color: "#ffffff", weight: edgeVisual.weight + 2, opacity: 0.6, dashArray: edgeVisual.dashArray, interactive: false }}
      />
      <Polyline
        positions={[[from.lat, from.lng], [to.lat, to.lng]]}
        pathOptions={{ color: edgeVisual.color, weight: edgeVisual.weight, opacity: 1, dashArray: edgeVisual.dashArray, interactive: false }}
      />
    </>
  );
});

MapMtEdgeComponent.displayName = "MapMtEdgeComponent";
...
const MapSelectorMtEdgesLayer: React.FC<MapSelectorMtEdgesLayerProps> = React.memo(({
  paneName,
  topology,
  popupTopology,
  polesById,
      ...
      {(topology.edges || []).map((edge) => {
        ...
        return (
          <MapMtEdgeComponent
            ...
            from={from}
            to={to}
            popupEdge={popupEdgesById.get(edge.id) ?? edge}
            popupFrom={popupPolesById.get(edge.fromPoleId) ?? from}
            popupTo={popupPolesById.get(edge.toPoleId) ?? to}
            locale={locale}
...
MapSelectorMtEdgesLayer.displayName = "MapSelectorMtEdgesLayer";
