...
// ─── Componente de Marcador de Transformador Individual (Memoizado) ──────────

interface TransformerMarkerProps {
  transformer: MapBtTransformer;
  ...
  onBtSetTransformerChangeFlag?: (transformerId: string, flag: "existing" | "new" | "remove" | "replace") => void;
  onBtDeleteTransformer?: (id: string) => void;
}

const MapBtTransformerMarker = React.memo(({
  transformer,
  btEditorMode,
  polesById,
  locale,
...
}: TransformerMarkerProps) => {
  const t = getBtTopologyPanelText(locale).poleVerification;

  const icon = React.useMemo(() => {
    const verified = !!transformer.verified;
    const flag = getTransformerChangeFlag(transformer);
    const bg = getFlagColor(flag, verified ? "#15803d" : "#7c3aed");
    const dataSource = transformer.dataSource;
    ...
    return L.divIcon({
      ...
      html: `<div style="position:relative;width:14px;height:14px;"><svg width="14" height="14" viewBox="0 0 24 24"><path d="M12 21L2 3h20L12 21Z" fill="${bg}" stroke="#ffffff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>${sourceBadge}</div>`,
      iconSize: [14, 14],
      iconAnchor: [7, 7],
    });
  }, [transformer]);
      ...
      eventHandlers={{
        click: () => {
          if ((btEditorMode === "add-edge" || btEditorMode === "add-transformer") && onBtMapClick) {
            ...
            if (linkedPole) {
              onBtMapClick({ lat: linkedPole.lat, lng: linkedPole.lng, label: linkedPole.title });
              return;
            }
            onBtMapClick({ lat: transformer.lat, lng: transformer.lng, label: transformer.title });
          }
        },
        dragend: (e) => {
      ...
      <Tooltip direction="top" offset={[0, -10]} opacity={0.9}>
        <div className="flex flex-col items-center">
          ...
          {layerConfig?.labels && typeof transformer.projectPowerKva === "number" && (
            <span className="text-xs font-black text-violet-700">{transformer.projectPowerKva} kVA</span>
          )}
        </div>
      </Tooltip>
      <Popup>
        <div className="text-xs">
          ...
          {typeof transformer.projectPowerKva === "number" && (
            <div className="mt-1 border-t border-slate-100 pt-1">
              <div className="font-bold text-violet-800">Potência: {transformer.projectPowerKva} kVA</div>
            </div>
          )}
          {onBtRenameTransformer && (
            <input
          ...
          {onBtSetTransformerChangeFlag && (
            <div className={POPUP_FLAG_GRID_CLASS}>
              {(["existing", "new", "replace", "remove"] as const).map((flag) => (
                <button
                  key={flag}
                  onClick={(e) => { e.preventDefault(); onBtSetTransformerChangeFlag(transformer.id, flag); }}
                  className={getFlagButtonClass(getTransformerChangeFlag(transformer) === flag, flag)}
                >
                  {flag === "new" ? t.flagNew : flag === "remove" ? t.flagRemove : flag === "replace" ? t.flagReplace : t.flagExisting}
                </button>
              ))}
            </div>
          )}
          <div className={POPUP_TOOLBAR_CLASS}>
            <button
              onClick={(e) => { e.preventDefault(); e.stopPropagation(); onBtDeleteTransformer?.(transformer.id); }}
...
MapBtTransformerMarker.displayName = "MapBtTransformerMarker";
...
const MapSelectorTransformersLayer: React.FC<MapSelectorTransformersLayerProps> = React.memo(({
  paneName,
  transformers,
  btEditorMode,
  polesById,
...
MapSelectorTransformersLayer.displayName = "MapSelectorTransformersLayer";
