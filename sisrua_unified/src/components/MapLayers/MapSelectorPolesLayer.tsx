  ...
  ({
    ...
    const eventHandlers = React.useMemo(
      () => ({
        ...
        dragstart: () => {
          if (onBtDragPoleRealtime)
            onBtDragPoleRealtime(pole.id, pole.lat, pole.lng);
        },
        drag: (e: any) => {
                      ...
                      <div className="mt-1 border-t border-slate-100 pt-1 pb-2">
                        ...
                        {popupPole.btStructures && (
                          <div className="text-[10px] text-slate-500">
                            Estruturas:{" "}
                            {Object.values(popupPole.btStructures)
                              .filter(Boolean)
                              .join(", ")}
                          </div>
                        )}
                    ...
                    <div className="grid grid-cols-2 gap-2 pt-2 border-t border-slate-100">
                      <button
                        ...
                        className={getIconActionButtonClass(
                          hasTransformer ? "amber" : "slate",
                        )}
                      >
                        <Zap
                          size={14}
                          className={hasTransformer ? "fill-current" : ""}
                        />
                      ...
                      >
                        <Minus
                          size={14}
                          className={pole.circuitBreakPoint ? "stroke-[4]" : ""}
                        />
    ...
    return (
      <Pane name={paneName} style={{ zIndex: 470 }}>
        {poles.map((pole) => {
          ...
          const dragPos =
            isDragged && draggedPole
              ? L.latLng(draggedPole.lat, draggedPole.lng)
              : null;
          const hasTransformer = !!poleHasTransformer.get(pole.id);

          const urbanViolation = flags?.enableNbr9050
            ? complianceResults?.urban?.find(
                (r: any) => r.poleId === pole.id && !r.conforme,
              )
            : null;
          const envViolation = flags?.enableEnvironmentalAudit
            ? complianceResults?.environmental?.find(
                (r: any) => r.poleId === pole.id,
              )
            : null;
          ...
          const solarViolation =
            ...
            (solarData?.nivelRiscoTermico === "alto" ||
              solarData?.nivelRiscoTermico === "critico");

          const complianceViolation =
            !!urbanViolation || !!envViolation || solarViolation;
          const complianceDetail =
            urbanViolation?.detalhe ||
            envViolation?.nomeArea ||
            (solarViolation
              ? `Risco Térmico: ${solarData.eficienciaPercent}%`
              : "");
