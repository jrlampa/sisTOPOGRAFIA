...
const MapBtPoleMarker = React.memo(({
  ...
  }, [hasTransformer, isLoadCenter, poleFlag, verified, isCritical, isPending, dataSource]);
  ...
  const isViolation =
    poleAccumulated?.cqtStatus === "CRÍTICO" || isCritical;

  return (
    <>
      {/* Círculo de Alcance (Ghost Range) durante o Arrasto */}
      {isDragged && dragPos && (
      ...
      {/* Rótulo Permanente de CQT nas Pontas */}
      ...
      <CircleMarker
        ...
        radius={isCritical ? 9 : isPending ? 8 : 7}
        pathOptions={{
          color: "#ffffff",
          weight: 2,
          fillColor: getFlagColor(poleFlag, verified ? "#16a34a" : "#2563eb"),
      ...
      <Marker
        ...
        eventHandlers={{
          ...
          dragend: (e) => {
            ...
            onBtDragPoleRealtime?.(pole.id, 0, 0);
          },
          drag: (e) => {
            const { lat, lng } = (e.target as L.Marker).getLatLng();
      ...
      >
        <Tooltip direction="top" offset={[0, -12]} opacity={1} className="bim-pop-in-tooltip">
          <div className="min-w-[140px] overflow-hidden rounded-2xl border border-white/20 bg-slate-900/40 p-3 shadow-2xl backdrop-blur-md dark:bg-zinc-950/60">
            <div className="mb-2 flex items-center justify-between border-b border-white/10 pb-1.5">
              ...
              {isViolation && <div className="h-1.5 w-1.5 rounded-full bg-red-500 animate-pulse" />}
            </div>
          ...
          <Popup>
            <div className="text-xs min-w-[200px]">
              ...
              {poleAccumulated && (
                <div className="space-y-1.5 bg-slate-50 p-2 rounded-lg mb-3 border border-slate-100">
                  ...
                  <div className="flex justify-between items-center text-[11px]">
                    ...
                    <span className={`font-black ${cqtClass}`}>{poleAccumulated.dvAccumPercent?.toFixed(2) ?? "-"}%</span>
                  </div>
                  <div className="flex justify-between items-center text-[11px]">
                ...
                <div className="mt-1 border-t border-slate-100 pt-1 pb-2">
                  ...
                  {popupPole.btStructures && (
                    <div className="text-sky-800 italic text-[10px]">
                      {t.structuresLabel} {[
                        popupPole.btStructures.si1, popupPole.btStructures.si2, popupPole.btStructures.si3, popupPole.btStructures.si4,
                      ].filter(Boolean).join(", ") || "-"}
                    </div>
                  )}
                </div>
              ...
              <div className="flex flex-col gap-1 text-[11px] border-t border-slate-100 pt-2 mt-1">
                <div className={`flex items-center gap-1.5 font-bold ${verified ? "text-green-600" : "text-amber-600"}`}>
                  {verified ? <CheckCircle size={12} /> : <CircleIcon size={12} />}
                  {verified ? t.flagExisting : "Pendente de Verificação"}
                </div>
                ...
                <div className={`${POPUP_FLAG_GRID_CLASS} mt-3`}>
                  {(["existing", "new", "replace", "remove"] as const).map((flag) => (
                    <button
                      ...
                      className={getFlagButtonClass(getPoleChangeFlag(popupPole) === flag, flag)}
                    >
                  ...
                  <button
                    onClick={(e) => {
                      e.preventDefault();
                      onBtTogglePoleCircuitBreak?.(pole.id, !(popupPole.circuitBreakPoint ?? false));
                    }}
              ...
              <div className={`${POPUP_TOOLBAR_CLASS} mt-4 pt-2 border-t border-slate-100`}>
                <button
                  onClick={(e) => { e.preventDefault(); e.stopPropagation(); onBtDeletePole?.(pole.id); }}
                ...
                <button
                  ...
                  className={getIconActionButtonClass("violet", !!hasTransformer)}
                  title="Alternar Transformador"
                >
                  <Triangle size={14} className="rotate-180 fill-current" />
                </button>
                {onBtQuickAddPoleRamal && (
                  <button onClick={() => onBtQuickAddPoleRamal(pole.id)} className={getIconActionButtonClass("sky")} title="Adicionar Ramal">
                    <Plus size={14} />
                  </button>
                )}
                {onBtQuickRemovePoleRamal && (
                  <button onClick={() => onBtQuickRemovePoleRamal(pole.id)} className={getIconActionButtonClass("slate")} title="Remover Ramal">
                    <Minus size={14} />
                  </button>
                )}
              </div>
...
// ─── Componente Principal da Camada ──────────────────────────────────────────
