...
// ─── Componente de Marcador MT Individual (Memoizado) ───────────────────────

interface MtPoleMarkerProps {
  pole: MapMtPole;
  popupPole: MapMtPole;
...
const MapMtPoleMarker = React.memo(({
  pole,
  popupPole,
  mtEditorMode,
  locale,
  ...
  }, [pole.nodeChangeFlag, pole.verified]);
  ...
  return (
    ...
    >
      <Tooltip permanent={mtEditorMode === "mt-add-edge"} direction="top" offset={[0, -10]} opacity={0.85}>
        <div className="flex flex-col items-center">
          <span className="text-xs font-bold text-orange-900 dark:text-orange-100">{pole.title}</span>
          {layerConfig?.labels && pole.mtStructures && (
            <span className="text-[8px] font-bold text-orange-700">
              {[pole.mtStructures.n1, pole.mtStructures.n2, pole.mtStructures.n3, pole.mtStructures.n4].filter(Boolean).join(", ")}
            </span>
          )}
        </div>
      </Tooltip>
      <Popup>
        <div className="text-xs">
          <div className="flex items-center justify-between gap-4">
            ...
            <span className={`text-xs font-bold ${popupPole.verified ? "text-green-600" : "text-orange-600"}`}>
              {popupPole.verified ? "VERIFICADO" : "PENDENTE"}
            </span>
          </div>
          <div className="mt-1 text-slate-500 font-mono text-xs">{popupPole.id}</div>
          {popupPole.mtStructures && (
            <div className="mt-1 border-t border-slate-100 pt-1">
              <div className="text-orange-800 italic">
                Estruturas MT: {[popupPole.mtStructures.n1, popupPole.mtStructures.n2, popupPole.mtStructures.n3, popupPole.mtStructures.n4].filter(Boolean).join(", ") || "-"}
              </div>
            </div>
          )}
          {onMtRenamePole && (
          ...
          <div className="mt-2 flex flex-col gap-2">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className={`rounded px-2 py-0.5 text-xs font-bold transition-colors ${popupPole.verified ? "bg-green-100 text-green-700 hover:bg-green-200" : "bg-orange-100 text-orange-700 hover:bg-orange-200"}`}
              >
            ...
            {onMtDeletePole && (
              <button
                onClick={(e) => { e.preventDefault(); e.stopPropagation(); onMtDeletePole(pole.id); }}
              ...
              >
                <Trash2 size={12} /> Excluir Poste MT
              </button>
            )}
          </div>
        </div>
...
MapMtPoleMarker.displayName = "MapMtPoleMarker";
...
const MapSelectorMtPolesLayer: React.FC<MapSelectorMtPolesLayerProps> = React.memo(({
  paneName,
  poles,
  popupPoles,
  mtEditorMode,
...
MapSelectorMtPolesLayer.displayName = "MapSelectorMtPolesLayer";
