...
import { FormFieldMessage, getValidationInputClassName } from './FormFieldFeedback';
import type { DgOptimizationOutput, DgScenario } from '../hooks/useDgOptimization';
import type { MtRouterState, MtSelectionMode, MtNetworkProfile } from '../hooks/useMtRouter';
import type {
  AppSettings,
  BtEditorMode,
  BtNetworkScenario,
...
const BtTopologyPanel = React.lazy(() => lazyWithRetry(() => import('./BtTopologyPanel')));
...
type TransformerDebugById = Record<string, { assignedClients: number; estimatedDemandKva: number }>;
  ...
  return (
    ...
    >
      ...
      >
        <Icon
          size={20}
          className={active ? colorClass.replace('ring-', 'text-').replace('/50', '') : ''}
        />
      </div>
      <span
        className={`text-[9px] font-black uppercase tracking-tighter ${active ? 'text-white' : 'text-slate-500'}`}
      >
        {label}
      </span>
    </button>
...
export interface SidebarBtEditorSectionProps {
  ...
  onMtRouterSetMtCqtParams?: (params: { voltageKv: number; cqtLimitFraction: number }) => void;
...
export function SidebarBtEditorSection({
  ...
  const currentTotalCableLengthMeters = useMemo(() => {
    return effectiveDgTopology.edges.reduce((sum, edge) => sum + (edge.lengthMeters ?? 0), 0);
  }, [effectiveDgTopology.edges]);

  const coordinateValidation = getCoordinateInputFeedback(btPoleCoordinateInput);
          ...
          >
            ...
            <div className="relative">
              ...
              <div className="absolute right-4 top-1/2 -translate-y-1/2">
                {btPoleCoordinateInput && (
                  <div
                    className={`w-2 h-2 rounded-full shadow-[0_0_8px] ${coordinateValidation.state === 'success' ? 'bg-emerald-500 shadow-emerald-500' : 'bg-rose-500 shadow-rose-500'}`}
                  />
                )}
              </div>
            </div>
          ...
          >
            ...
            <Suspense fallback={<InlineSuspenseFallback label={t.loadingBtPanel} />}>
              <DgOptimizationPanel
                ...
                hasProjectedPoles={effectiveDgTopology.poles.some(p => p.nodeChangeFlag === 'new')}
        ...
        <div className="pt-4">
          ...
          <Suspense fallback={<InlineSuspenseFallback label={t.loadingBtPanel} />}>
            <MtRouterPanel
              ...
              onSetNetworkProfile={onMtRouterSetNetworkProfile ?? (() => undefined)}
