...
class DXFStyleManager:
    ...
    def setup_linetypes(doc):
        ...
        if "DASHED" not in doc.linetypes:
            doc.linetypes.new(
                "DASHED",
                dxfattribs={"description": "Dashed", "pattern": [0.75, 0.5, -0.25]},
            )
        if "HIDDEN" not in doc.linetypes:
            doc.linetypes.new(
                "HIDDEN",
                dxfattribs={"description": "Hidden", "pattern": [0.375, 0.25, -0.125]},
            )
    ...
    def setup_layers(doc):
        ...
        for name, color, lineweight in layers:
            if name not in doc.layers:
                doc.layers.new(
                    name,
                    dxfattribs={"color": color, "lineweight": map_weight(lineweight)},
                )
    ...
    def setup_blocks(doc):
        ...
        if "ARVORE" not in doc.blocks:
            ...
            blk.add_circle(
                (0, 0), radius=2, dxfattribs={"layer": "VEGETACAO", "color": 3}
            )
        ...
        if "POSTE" not in doc.blocks:
            ...
            blk.add_attdef(
                "ID", (0.5, 0.5), dxfattribs={"height": 0.3, "color": 2}
            ).dxf.text = "000"
            blk.add_attdef(
                "TYPE", (0.5, 0.1), dxfattribs={"height": 0.2, "color": 8}
            ).dxf.text = "POLE"
            blk.add_attdef(
        ...
        if "BT_POSTE" not in doc.blocks:
            ...
            blk.add_circle(
                (0, 0), radius=0.55, dxfattribs={"layer": "BT_POSTES", "color": 2}
            )
            blk.add_line(
                (-0.4, 0), (0.4, 0), dxfattribs={"layer": "BT_POSTES", "color": 2}
            )
            blk.add_line(
        ...
        if "BT_TRAFO_INV" not in doc.blocks:
            ...
            blk.add_lwpolyline(
                [(-1.3, 1.1), (1.3, 1.1), (0, -1.7)],
                close=True,
                dxfattribs={"layer": "BT_TRAFOS", "color": 3},
            )
            blk.add_line(
        ...
        if "BANCO" not in doc.blocks:
            ...
            blk.add_lwpolyline(
                [(-0.8, -0.4), (0.8, -0.4), (0.8, 0.4), (-0.8, 0.4)], close=True
            )
        ...
        if "NORTE" not in doc.blocks:
            ...
            blk.add_lwpolyline(
                [(0, 0), (-1, -3), (0, 1), (1, -3)], close=True, dxfattribs={"color": 7}
            )
            blk.add_text("N", dxfattribs={"height": 1.5, "color": 7}).set_placement(
                (0, 1.5), align=TextEntityAlignment.CENTER
            )
        ...
        if "ESCALA" not in doc.blocks:
            ...
            blk.add_text("0", dxfattribs={"height": 1}).set_placement(
                (0, -1.5), align=TextEntityAlignment.CENTER
            )
            blk.add_text("5m", dxfattribs={"height": 1}).set_placement(
                (5, -1.5), align=TextEntityAlignment.CENTER
            )
            blk.add_text("10m", dxfattribs={"height": 1}).set_placement(
                (10, -1.5), align=TextEntityAlignment.CENTER
            )
    ...
    def setup_logo(doc):
        ...
        if "LOGO" not in doc.blocks:
            ...
            blk.add_lwpolyline(
                [(0, 5), (5, 0), (0, -5), (-5, 0)], close=True, dxfattribs={"color": 7}
            )
            blk.add_circle((0, 0), radius=1, dxfattribs={"color": 2})
            blk.add_text("RUAS", dxfattribs={"height": 1.5, "color": 7}).set_placement(
                (0, -7), align=TextEntityAlignment.CENTER
            )
