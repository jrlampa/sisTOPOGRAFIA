...
async function setupDeterministicMocks(page: import("@playwright/test").Page) {
  ...
  await page.route("**/api/analyze", async (route) => {
    await route.fulfill({
      status: 500,
      contentType: "application/json",
      body: JSON.stringify({ error: "Ollama not available" }),
    });
  });
}

/**
 * E2E Test for Ollama AI Analysis Error Handling
 *
 * This test verifies that when Ollama is not available,
 * the application shows a helpful error message to the user.
 */
test.describe("Ollama AI Analysis Error Handling", () => {
  test.describe.configure({ mode: "serial" });

  test("should keep UI responsive when AI analysis endpoint fails", async ({
    page,
  }) => {
  ...
  test("should not crash when Ollama AI analysis fails", async ({ page }) => {
    ...
    const hasCriticalError = pageErrors.some(
      (err) =>
...
test.describe("DXF Generation with Real Coordinates", () => {
  test.skip(
    !runIntegration,
    "Set E2E_RUN_INTEGRATION=true to run live DXF integration test.",
  );

  test("should initiate DXF generation for Brasil coordinates", async ({
    page,
  }) => {
    ...
    const radiusInput = page
      .locator('input[placeholder*="Raio"]')
    ...
    if (await exportButton.isVisible()) {
      ...
      const hasDownloadLink = await page
        ...
        .isVisible()
        .catch(() => false);
      const hasJobStatus = await page
        .locator("text=/processando|gerando|queued/i")
        .isVisible()
        .catch(() => false);
      ...
      if (hasDownloadLink) {
        ...
        console.log(
          "⚠️  DXF generation failed (expected if no internet/backend)",
        );
      } else {
