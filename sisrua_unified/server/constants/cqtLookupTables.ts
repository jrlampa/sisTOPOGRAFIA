...
export const getTrafosZByScenario = (
  ...
  if (config.useDbConstantsCqt) {
    const cached = constantsService.getSync<TrafosZRow[]>(
      "cqt",
      "TRAFOS_Z_BASELINE",
    );
...
export const getCabosByScenario = (
  ...
  if (config.useDbConstantsCqt) {
    const cached = constantsService.getSync<CaboLookupRow[]>(
      "cqt",
      "CABOS_BASELINE",
    );
    if (cached) return cached;
...
export const getDisjuntoresByScenario = (
  ...
  if (config.useDbConstantsCqt) {
    const cached = constantsService.getSync<DisjuntorLookupRow[]>(
      "cqt",
