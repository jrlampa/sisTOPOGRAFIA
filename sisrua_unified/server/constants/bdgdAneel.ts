...
const TIP_CONDUT_CODES = new Set(
    Object.keys(ANEEL_TIP_CONDUT).map(Number),
);
const MAT_CONDU_CODES = new Set(
    Object.keys(ANEEL_MAT_CONDU).map(Number),
);
const MAT_ESTR_CODES = new Set(
    Object.keys(ANEEL_MAT_ESTR).map(Number),
);
const TIP_TRAF_CODES = new Set(
    Object.keys(ANEEL_TIP_TRAF).map(Number),
);
const SITU_CODES = new Set(
    Object.keys(ANEEL_SITU).map(Number),
);
...
const SEGBT_DEF: BdgdLayerDef = {
    ...
    fields: [
        ...
        { name: 'FAS_CON',   type: 'string',  required: true,  allowedCodes: ANEEL_FAS_CON },
        { name: 'COMP',      type: 'number',  required: true,  min: 0.1, max: 5000 },
        { name: 'TIP_CONDUT',type: 'number',  required: true,  allowedCodes: TIP_CONDUT_CODES },
        { name: 'MAT_CONDU', type: 'number',  required: true,  allowedCodes: MAT_CONDU_CODES },
...
const PONNOT_DEF: BdgdLayerDef = {
    ...
    fields: [
        { name: 'COD_ID',   type: 'string',  required: true,  maxLength: 25 },
        { name: 'DES_CONC', type: 'string',  required: true,  maxLength: 10 },
        { name: 'MAT_ESTR', type: 'number',  required: true,  allowedCodes: MAT_ESTR_CODES },
...
const EQTRAT_DEF: BdgdLayerDef = {
    ...
    fields: [
        ...
        { name: 'TIP_TRAF', type: 'number',  required: true,  allowedCodes: TIP_TRAF_CODES },
        { name: 'SITU',     type: 'number',  required: false, allowedCodes: SITU_CODES },
        { name: 'geometry', type: 'coordinate', required: true },
    ],
};
