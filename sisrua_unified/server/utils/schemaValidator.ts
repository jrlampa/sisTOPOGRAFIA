...
function getDefinition(root: SchemaDoc, ref: string): SchemaDoc {
    ...
    for (const part of parts) {
        if (typeof cur !== 'object' || cur === null) throw new Error(`$ref path broken at: ${part}`);
...
function validateNode(
    ...
    if (Array.isArray(schema['oneOf'])) {
        ...
        if (passing.length !== 1) {
            errors.push(`${pointer}: must match exactly one of ${branches.length} schemas (matched ${passing.length})`);
        }
        return;
    }
    ...
    if (Array.isArray(schema['anyOf'])) {
        ...
        if (!passing) {
            errors.push(`${pointer}: must match at least one of ${branches.length} schemas`);
        }
        return;
    }
    ...
    if ('type' in schema) {
        ...
        const actualType = data === null ? 'null' : typeof data === 'object' ? (Array.isArray(data) ? 'array' : 'object') : typeof data;
        if (!types.includes(actualType)) {
            errors.push(`${pointer}: expected type ${types.join('|')}, got ${actualType}`);
            return;
        }
    }
    ...
    if (Array.isArray(schema['enum'])) {
        ...
        if (!allowed.includes(data)) {
            errors.push(`${pointer}: must be one of ${JSON.stringify(allowed)}, got ${JSON.stringify(data)}`);
        }
    }
    ...
    if (typeof data === 'string') {
        if (typeof schema['minLength'] === 'number' && data.length < schema['minLength']) {
            errors.push(`${pointer}: minLength ${schema['minLength']}, got ${data.length}`);
        }
        if (typeof schema['maxLength'] === 'number' && data.length > schema['maxLength']) {
            errors.push(`${pointer}: maxLength ${schema['maxLength']}, got ${data.length}`);
        }
    ...
    if (typeof data === 'number') {
        ...
        if (typeof schema['exclusiveMinimum'] === 'number' && data <= schema['exclusiveMinimum']) {
            errors.push(`${pointer}: exclusiveMinimum ${schema['exclusiveMinimum']}, got ${data}`);
        }
        if (typeof schema['exclusiveMaximum'] === 'number' && data >= schema['exclusiveMaximum']) {
            errors.push(`${pointer}: exclusiveMaximum ${schema['exclusiveMaximum']}, got ${data}`);
        }
    }
    ...
    if (Array.isArray(data)) {
        if (typeof schema['minItems'] === 'number' && data.length < schema['minItems']) {
            errors.push(`${pointer}: minItems ${schema['minItems']}, got ${data.length}`);
        }
        if (typeof schema['maxItems'] === 'number' && data.length > schema['maxItems']) {
            errors.push(`${pointer}: maxItems ${schema['maxItems']}, got ${data.length}`);
        }
    ...
    if (typeof data === 'object' && data !== null && !Array.isArray(data)) {
        ...
        // properties
        const propsSchema = schema['properties'] as Record<string, SchemaDoc> | undefined;
        if (propsSchema) {
