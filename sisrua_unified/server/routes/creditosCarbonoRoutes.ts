...
import { z } from "zod";
import {
  CreditosCarbonoService,
  TipoAcaoReducao,
} from "../services/creditosCarbonoService.js";
...
// ─── POST /calculos ───────────────────────────────────────────────────────────
router.post("/calculos", (req: Request, res: Response) => {
...
router.get("/calculos", (req: Request, res: Response) => {
...
router.post("/calculos/:id/certificar", (req: Request, res: Response) => {
  ...
  if (!calc) {
    return res.status(422).json({ erro: "Cálculo não encontrado ou não está no status 'calculado'" });
  }
  return res.json(calc);
});
