...
const btTransformerReadingSchema = z
  .object({
...
const btEdgeSchema = z
  .object({
    ...
    cqtLengthMeters: z.coerce.number().optional(),
    conductors: z
      .array(
        z
          .object({
...
router.post("/derived", (req: Request, res: Response) => {
  try {
    ...
    return res
      .status(500)
      .json({ error: "Failed to compute BT derived state" });
  }
});
