...
const basesLegaisValidas: BaseLegal[] = [
    'consentimento', 'cumprimento_obrigacao', 'execucao_politica',
    'estudos_orgao', 'execucao_contrato', 'exercicio_direitos',
    'protecao_vida', 'tutela_saude', 'interesse_legitimo', 'protecao_credito',
];

const categoriasValidas: CategoriaDado[] = [
    'identificacao', 'contato', 'localizacao', 'profissional',
    'tecnico', 'sensivel_saude', 'sensivel_biometrico',
];

const direitosValidos: DireitoTitular[] = [
    'confirmacao_existencia', 'acesso', 'correcao', 'anonimizacao_bloqueio',
    'portabilidade', 'eliminacao', 'informacao_compartilhamento',
    'revogacao_consentimento', 'oposicao',
];

const statusValidos: StatusSolicitacao[] = ['recebida', 'em_analise', 'atendida', 'indeferida'];

const tiposIncidente: TipoIncidente[] = [
    'acesso_nao_autorizado', 'divulgacao_indevida', 'alteracao_nao_autorizada',
    'perda_destruicao', 'ransomware', 'phishing', 'vazamento_interno', 'outro',
];

const severidades: SeveridadeIncidente[] = ['baixa', 'media', 'alta', 'critica'];

const etapasPlaybook: EtapaPlaybook[] = [
    'deteccao_triagem', 'contencao_imediata', 'avaliacao_impacto',
    'notificacao_anpd', 'comunicacao_titulares', 'remediacao_licoes',
];
...
const fluxoSchema = z.object({
    ...
    categorias: z.array(z.enum(categoriasValidas as [CategoriaDado, ...CategoriaDado[]])).min(1),
...
const incidenteSchema = z.object({
    ...
    severidade: z.enum(severidades as [SeveridadeIncidente, ...SeveridadeIncidente[]]),
...
router.post('/fluxos', (req: Request, res: Response) => {
    ...
    logger.info('LGPD: fluxo de tratamento registrado', { id: fluxo.id, nome: fluxo.nome });
...
router.post('/direitos', (req: Request, res: Response) => {
    ...
    const sol = registrarSolicitacaoDireito(titularId, direito, descricao, fluxoId);
    logger.info('LGPD: solicitação de direito registrada', { id: sol.id, direito, titularId });
...
router.put('/direitos/:id/status', (req: Request, res: Response) => {
    ...
    const updated = atualizarSolicitacao(req.params.id, parsed.data.status, parsed.data.resposta);
    if (!updated) return res.status(404).json({ erro: 'Solicitação não encontrada' });
...
router.post('/incidentes', (req: Request, res: Response) => {
    ...
    logger.info('LGPD: incidente registrado', { id: inc.id, tipo: inc.tipo, severidade: inc.severidade });
...
router.put('/incidentes/:id/etapa', (req: Request, res: Response) => {
    ...
    const updated = concluirEtapa(req.params.id, parsed.data.etapa, parsed.data.evidencia);
    if (!updated) return res.status(404).json({ erro: 'Incidente não encontrado' });
    return res.json(updated);
});
