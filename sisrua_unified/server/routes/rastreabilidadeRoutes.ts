...
router.post("/", (req: Request, res: Response) => {
  const { requisito, implementacoes, testes, status, observacao } =
    req.body as Record<string, unknown>;
  if (!requisito || !implementacoes || !testes || !status) {
    return res
      .status(400)
      .json({
        error: "requisito, implementacoes, testes e status são obrigatórios",
      });
  }
  const item = adicionarItem({
    requisito,
    implementacoes,
    testes,
    status,
    observacao,
  } as Parameters<typeof adicionarItem>[0]);
  return res.status(201).json(item);
});

router.patch("/:id/status", (req: Request, res: Response) => {
  const { status, observacao } = req.body as {
  ...
  if (!ok)
    return res
      .status(404)
      .json({ error: "Item não encontrado ou é canônico (imutável)" });
