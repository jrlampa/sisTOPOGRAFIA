...
router.get('/:id/integridade', (req: Request, res: Response) => {
  ...
  if (!resultado.hashEsperado) return res.status(404).json({ error: 'Pacote não encontrado' });
...
router.post('/:id/emitir', (req: Request, res: Response) => {
  ...
  if (!pacote) return res.status(422).json({ error: 'Pacote deve estar validado antes de emitir' });
  return res.json(pacote);
});
