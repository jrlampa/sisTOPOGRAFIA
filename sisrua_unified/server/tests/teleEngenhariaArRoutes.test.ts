...
describe("teleEngenhariaArRoutes", () => {
  ...
  it("POST /sessoes/:id/participantes adiciona participante", async () => {
    ...
    const res = await request(app)
      .post(`${BASE}/sessoes/te-1/participantes`)
      .send({ usuarioId: "u1", nomeUsuario: "Maria Silva", papel: "operador" });
    expect(res.status).toBe(201);
  ...
  it("POST /sessoes/:id/anotacoes registra quando sessão ativa", async () => {
    ...
    await request(app)
      ...
      .send({ usuarioId: "u1", nomeUsuario: "Maria Silva", papel: "operador" });
    const res = await request(app)
      .post(`${BASE}/sessoes/te-1/anotacoes`)
      .send({
        participanteId: "tp-1",
        tipoAnotacao: "linha",
  ...
  it("POST /sessoes/:id/anotacoes retorna 422 se não ativa", async () => {
    ...
    const res = await request(app)
      .post(`${BASE}/sessoes/te-1/anotacoes`)
      .send({ participanteId: "tp-1", tipoAnotacao: "linha", geometria: {} });
  ...
  it("PATCH /sessoes/:id/sincronia atualiza estado", async () => {
    ...
    const res = await request(app)
      .patch(`${BASE}/sessoes/te-1/sincronia`)
      .send({ estadoSincronia: "degradado" });
    expect(res.status).toBe(200);
