...
import {
  ...
  TRAFOS_Z_BASELINE,
} from "../constants/cqtLookupTables";
import {
...
describe("cqtEngine.calculateDbIndicators", () => {
  it("reproduces baseline values for K6/K7/K8/K10", () => {
    ...
    expect(result.k7DemAtual).toBeCloseTo(
      CQT_BASELINE_TARGETS.db.k7DemAtual,
      12,
    );
...
describe("cqtEngine protection layer", () => {
  it("calculates Ib for MONO, BIF and TRI phases", () => {
    ...
    // BIF: S*1000 / (2*Vfase*eta) — ABNT NBR 5410 §6.5, 2 fases + neutro, 127 V fase-neutro
    const bifIb = calculateIb({
      fase: "BIF",
      acumuladaKva: 10,
      eta: 1,
      tensaoTrifasicaV: 127,
    ...
    // 10000 / (2*127) ≈ 39.37 A — lower than MONO because two phases share the load
    expect(bifIb).toBeCloseTo((10 * 1000) / (2 * 127), 10);
  ...
  it("supports QT-PONTO using projected impedance with power factor", () => {
    const modulusQt = calculateQtPonto({
      fase: "TRI",
      acumuladaKva: 12,
      correctedResistance: 0.46,
      reactance: 0.12,
    ...
    const projectedQt = calculateQtPonto({
      fase: "TRI",
      acumuladaKva: 12,
      correctedResistance: 0.46,
      reactance: 0.12,
    ...
    expect(projectedQt).toBeGreaterThan(0);
    expect(projectedQt).toBeLessThan(modulusQt);
  });

  it("returns fallback 0 when QT-PONTO power factor is invalid", () => {
    const qt = calculateQtPonto({
      fase: "TRI",
      acumuladaKva: 12,
      correctedResistance: 0.46,
