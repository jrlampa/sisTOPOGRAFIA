...
jest.mock('../services/btRadialCalculationService', () => ({
    calculateBtRadial: calculateMock,
    BtRadialValidationError: BtRadialValidationErrorMock,
}));

// Mock btCatalogService
jest.mock('../services/btCatalogService', () => ({
    getBtCatalog: jest.fn(() => ({ conductors: [], transformers: [], version: { version: '1.0.0', checksum: 'abc', definedAt: '2026-01-01' } })),
    getCatalogVersion: jest.fn(() => ({ version: '1.0.0', checksum: 'abc', definedAt: '2026-01-01' })),
}));
...
const validBody = () => ({
    transformer: { id: 'TR225', rootNodeId: 'R', kva: 225, zPercent: 0.035, qtMt: 0.0183 },
    nodes: [
        { id: 'R', load: { localDemandKva: 0 } },
        { id: 'A', load: { localDemandKva: 10 } },
    ],
    edges: [{ fromNodeId: 'R', toNodeId: 'A', conductorId: '95 Al - Arm', lengthMeters: 50 }],
    phase: 'TRI',
});
...
const mockCalculationResult = () => ({
    ...
    nodeResults: [],
    terminalResults: [],
    worstCase: { worstTerminalNodeId: 'A', cqtGlobal: 0.035, criticalPath: ['R', 'A'], qtTrafo: 0.035 },
    totalDemandKva: 10,
    consistencyAlerts: [],
});
