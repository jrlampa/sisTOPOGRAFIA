...
describe("provenienciaForenseRoutes", () => {
  it("POST /dossies cria dossiê", async () => {
    const res = await request(app)
      .post(`${BASE}/dossies`)
      .send({ tenantId: "t1", projetoId: "p1", titulo: "Dossie Forense" });
  ...
  it("GET /dossies filtra por tenant", async () => {
    await request(app)
      .post(`${BASE}/dossies`)
      .send({ tenantId: "t1", projetoId: "p1", titulo: "Dossie A" });
    await request(app)
      .post(`${BASE}/dossies`)
      .send({ tenantId: "t2", projetoId: "p2", titulo: "Dossie B" });
  ...
  it("POST /dossies/:id/artefatos adiciona artefato", async () => {
    await request(app)
      ...
      .send({ tenantId: "t1", projetoId: "p1", titulo: "Dossie A" });
    const res = await request(app)
      .post(`${BASE}/dossies/pf-1/artefatos`)
      .send({
  ...
  it("POST /dossies/:id/selo-temporal sela dossiê", async () => {
    ...
    const res = await request(app)
      .post(`${BASE}/dossies/pf-1/selo-temporal`)
      .send({ provedor: "rfc3161_homologado" });
    expect(res.status).toBe(200);
  ...
  it("POST /dossies/:id/assinar-icp assina dossiê selado", async () => {
    ...
    await request(app)
      ...
      .send({ provedor: "rfc3161_homologado" });
    const res = await request(app)
      .post(`${BASE}/dossies/pf-1/assinar-icp`)
      .send({ certificadoSerial: "SERIAL-12345" });
    expect(res.status).toBe(200);
  ...
  it("GET /dossies/:id/verificar-integridade retorna íntegro", async () => {
    ...
    const res = await request(app).get(
      `${BASE}/dossies/pf-1/verificar-integridade`,
    );
    expect(res.status).toBe(200);
