...
describe("Multi-tenant Isolation", () => {
  it("POST /tenants — registra novo tenant com isolamento strict", async () => {
    const res = await request(app)
      .post("/api/tenant-isolation/tenants")
  ...
  it("GET /tenants — lista todos os tenants", async () => {
    await request(app).post("/api/tenant-isolation/tenants").send({ tenantId: "t1" });
    await request(app).post("/api/tenant-isolation/tenants").send({ tenantId: "t2" });
    const res = await request(app).get("/api/tenant-isolation/tenants");
    expect(res.status).toBe(200);
    expect(res.body.length).toBe(2);
  });

  it("GET /tenants/:tenantId — retorna perfil de isolamento", async () => {
    await request(app).post("/api/tenant-isolation/tenants").send({ tenantId: "tenant-gamma" });
    const res = await request(app).get("/api/tenant-isolation/tenants/tenant-gamma");
    expect(res.status).toBe(200);
    expect(res.body.tenantId).toBe("tenant-gamma");
  });

  it("GET /tenants/:tenantId — 404 para tenant desconhecido", async () => {
    const res = await request(app).get("/api/tenant-isolation/tenants/desconhecido");
    expect(res.status).toBe(404);
  });

  it("PUT /tenants/:tenantId/level — atualiza nível de isolamento", async () => {
    await request(app).post("/api/tenant-isolation/tenants").send({ tenantId: "tenant-delta" });
    const res = await request(app)
      .put("/api/tenant-isolation/tenants/tenant-delta/level")
      .send({ level: "relaxed" });
    expect(res.status).toBe(200);
  ...
  it("POST /verificar — permite acesso do mesmo tenant", async () => {
    await request(app).post("/api/tenant-isolation/tenants").send({ tenantId: "tenant-A" });
    const res = await request(app)
      .post("/api/tenant-isolation/verificar")
  ...
  it("POST /tenants/:tenantId/rotacionar-chave — gera nova chave de criptografia", async () => {
    await request(app).post("/api/tenant-isolation/tenants").send({ tenantId: "tenant-E" });
    const { body: antes } = await request(app).get("/api/tenant-isolation/tenants/tenant-E");
    const res = await request(app).post("/api/tenant-isolation/tenants/tenant-E/rotacionar-chave");
    expect(res.status).toBe(200);
  ...
  it("GET /relatorio — retorna relatório de isolamento", async () => {
    await request(app).post("/api/tenant-isolation/tenants").send({ tenantId: "t1", level: "strict" });
    await request(app).post("/api/tenant-isolation/tenants").send({ tenantId: "t2", level: "standard" });
    const res = await request(app).get("/api/tenant-isolation/relatorio");
    expect(res.status).toBe(200);
