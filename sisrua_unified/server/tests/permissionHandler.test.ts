...
const mockGetUserRole =
  vi.fn<(userId: string | undefined) => Promise<UserContext>>();
...
describe("PermissionHandler Middleware", () => {
  ...
  describe("admin role", () => {
    ...
    it("should allow admin to perform export_dxf", async () => {
      mockReq.headers!["x-user-id"] = "admin-user";
      await requirePermission("export_dxf")(
        mockReq as Request,
        mockRes as Response,
        nextFunction,
      );
    ...
    it("should allow admin to perform bt_calculate", async () => {
      mockReq.headers!["x-user-id"] = "admin-user";
      await requirePermission("bt_calculate")(
        mockReq as Request,
        mockRes as Response,
        nextFunction,
      );
      ...
      expect(nextFunction).toHaveBeenCalledWith(
        expect.objectContaining({
          message: expect.stringContaining(
            "Missing required permission: delete",
          ),
        }),
      );
    });
  ...
  describe("viewer role", () => {
    ...
    it("should deny viewer from export_dxf", async () => {
      ...
      expect(nextFunction).toHaveBeenCalledWith(
        expect.objectContaining({
          message: expect.stringContaining(
            "Missing required permission: export_dxf",
          ),
        }),
      );
    });
  ...
  describe("error handling", () => {
    ...
    it("should prefer res.locals.userId over header userId", async () => {
      mockGetUserRole.mockResolvedValue({ role: "admin", tenantId: null });
      ...
      expect(mockGetUserRole).toHaveBeenCalledWith("local-user");
      expect(nextFunction).toHaveBeenCalledWith();
    });
