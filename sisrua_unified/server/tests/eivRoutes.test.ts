...
const WRITER_HEADER = { 'x-user-id': 'eiv-writer' };
...
describe('EIV — /api/eiv', () => {
  describe('POST /estudos', () => {
    ...
    it('cria estudo com dados válidos → 201', async () => {
      const res = await request(app).post('/api/eiv/estudos').set(WRITER_HEADER).send({
        tenantId: 'tenant-eiv',
        titulo: 'EIV Subestação Zona Sul',
        empreendimento: 'Subestação MT/BT 500 kVA',
        municipio: 'São Paulo',
  ...
  describe('GET /estudos', () => {
    ...
    it('filtra por tenantId', async () => {
      await request(app).post('/api/eiv/estudos').set(WRITER_HEADER).send({
        tenantId: 'tA',
        titulo: 'EIV Tenant A',
        empreendimento: 'Obra A',
        municipio: 'SP',
        uf: 'SP',
        zonaUrbana: 'zona_comercial',
        areaImpactoM2: 300,
        populacaoAfetada: 200,
        responsavel: 'Eng A',
      });
      await request(app).post('/api/eiv/estudos').set(WRITER_HEADER).send({
        tenantId: 'tB',
        titulo: 'EIV Tenant B',
        empreendimento: 'Obra B',
        municipio: 'RJ',
        uf: 'RJ',
        zonaUrbana: 'zona_industrial',
        areaImpactoM2: 500,
        populacaoAfetada: 400,
        responsavel: 'Eng B',
      });
  ...
  describe('POST /estudos/:id/impactos', () => {
    beforeEach(async () => {
      await request(app).post('/api/eiv/estudos').set(WRITER_HEADER).send({
        tenantId: 't1',
        titulo: 'EIV Base',
        empreendimento: 'Obra Base',
        municipio: 'BH',
        uf: 'MG',
        zonaUrbana: 'zona_residencial',
        areaImpactoM2: 1000,
        populacaoAfetada: 800,
    ...
    it('rejeita dimensão duplicada → 422', async () => {
      await request(app).post('/api/eiv/estudos/eiv-1/impactos').set(WRITER_HEADER).send({
        dimensao: 'trafego',
        nivel: 'baixo',
  ...
  describe('POST /estudos/:id/calcular', () => {
    it('calcula EIV e retorna IEV → 200', async () => {
      await request(app).post('/api/eiv/estudos').set(WRITER_HEADER).send({
        tenantId: 't1',
        titulo: 'EIV Calcular',
        empreendimento: 'Subestação Calcular',
        municipio: 'Curitiba',
        uf: 'PR',
        zonaUrbana: 'zona_mista',
        areaImpactoM2: 1500,
        populacaoAfetada: 1000,
        responsavel: 'Eng C',
      });
      await request(app)
        .post('/api/eiv/estudos/eiv-1/impactos')
      ...
      const res = await request(app).post('/api/eiv/estudos/eiv-1/calcular').set(WRITER_HEADER);
      expect(res.status).toBe(200);
  ...
  describe('POST /estudos/:id/publicar', () => {
    it('publica estudo calculado → 200', async () => {
      ...
      await request(app).post('/api/eiv/estudos/eiv-1/calcular').set(WRITER_HEADER);
      const res = await request(app).post('/api/eiv/estudos/eiv-1/publicar').set(WRITER_HEADER);
      expect(res.status).toBe(200);
