...
const BASE = '/api/as-built-mobile';
const WRITER_HEADER = { 'x-user-id': 'asbuilt-writer' };
...
describe('GET /registros', () => {
  ...
  it('filtra por tenantId', async () => {
    await request(app)
      .post(`${BASE}/registros`)
      .set(WRITER_HEADER)
      .send({ ...registroPayload, tenantId: 'tA' });
    await request(app)
      .post(`${BASE}/registros`)
      .set(WRITER_HEADER)
      .send({ ...registroPayload, tenantId: 'tB' });
...
describe('GET /registros/:id', () => {
  ...
  it('retorna registro existente', async () => {
    await request(app).post(`${BASE}/registros`).set(WRITER_HEADER).send(registroPayload);
...
describe('POST /registros/:id/sincronizar', () => {
  it('sincroniza registro com desvios', async () => {
    ...
    await request(app)
      ...
      .set(WRITER_HEADER)
      .send(desvioPayload);
    const res = await request(app).post(`${BASE}/registros/ab-1/sincronizar`).set(WRITER_HEADER);
    expect(res.status).toBe(200);
...
describe('POST /registros/:id/aprovar', () => {
  it('aprova registro sincronizado com hash de integridade', async () => {
    ...
    await request(app)
      ...
      .set(WRITER_HEADER)
      .send(desvioPayload);
    await request(app).post(`${BASE}/registros/ab-1/sincronizar`).set(WRITER_HEADER);
    const res = await request(app)
