...
describe("Knowledge Base Routes (119)", () => {
  describe("GET /api/knowledge-base/articles", () => {
    it("deve retornar artigos pré-semeados", async () => {
      const res = await request(app).get("/api/knowledge-base/articles");
    ...
    it("deve filtrar por categoria seguranca", async () => {
      const res = await request(app).get(
        "/api/knowledge-base/articles?category=seguranca",
      );
      expect(res.status).toBe(200);
      expect(Array.isArray(res.body)).toBe(true);
      expect(
    ...
    it("deve filtrar por preventionAutomated=true", async () => {
      const res = await request(app).get(
        "/api/knowledge-base/articles?preventionAutomated=true",
      );
      expect(res.status).toBe(200);
      expect(
        res.body.every(
  ...
  describe("GET /api/knowledge-base/search", () => {
    it("deve encontrar artigo sobre OOM", async () => {
      const res = await request(app).get("/api/knowledge-base/search?q=oom");
      expect(res.status).toBe(200);
      expect(Array.isArray(res.body)).toBe(true);
  ...
  describe("GET /api/knowledge-base/articles/recurrence", () => {
    it("deve retornar relatório de recorrências", async () => {
      const res = await request(app).get(
        "/api/knowledge-base/articles/recurrence",
      );
      expect(res.status).toBe(200);
  ...
  describe("GET /api/knowledge-base/articles/:id", () => {
    it("deve retornar artigo pré-semeado por ID", async () => {
      const res = await request(app).get("/api/knowledge-base/articles/kb-dxf-001");
      expect(res.status).toBe(200);
    ...
    it("deve retornar 404 para artigo inexistente", async () => {
      const res = await request(app).get(
        "/api/knowledge-base/articles/kb-inexistente",
      );
  ...
  describe("POST /api/knowledge-base/articles/:id/occurrence", () => {
    it("deve incrementar occurrenceCount do artigo", async () => {
      const initial = await request(app).get(
        "/api/knowledge-base/articles/kb-dxf-001",
      );
      const countBefore = initial.body.occurrenceCount;

      const res = await request(app).post(
        "/api/knowledge-base/articles/kb-dxf-001/occurrence",
      );
      expect(res.status).toBe(200);
    ...
    it("deve retornar 422 para artigo inexistente", async () => {
      const res = await request(app).post(
        "/api/knowledge-base/articles/kb-inexistente/occurrence",
      );
