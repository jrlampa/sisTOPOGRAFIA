...
describe("GET /api/sombreamento/analises", () => {
  it("deve listar análises", async () => {
    await request(app).post("/api/sombreamento/analises").send(BASE_ANALISE);
    const res = await request(app).get("/api/sombreamento/analises");
...
describe("GET /api/sombreamento/analises/:id", () => {
  it("deve retornar 404 para id inexistente", async () => {
    const res = await request(app).get("/api/sombreamento/analises/sa-999");
...
describe("POST /api/sombreamento/analises/:id/aprovar", () => {
...
describe("GET /api/sombreamento/tipos-ativo", () => {
  it("deve listar tipos incluindo painel_solar", async () => {
    const res = await request(app).get("/api/sombreamento/tipos-ativo");
    expect(res.status).toBe(200);
