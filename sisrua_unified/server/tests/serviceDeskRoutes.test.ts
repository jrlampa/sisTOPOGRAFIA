...
describe("Service Desk Routes (113)", () => {
  describe("GET /api/service-desk/metrics", () => {
    it("deve retornar métricas do service desk", async () => {
      const res = await request(app).get("/api/service-desk/metrics");
  ...
  describe("GET /api/service-desk/sla-alerts", () => {
    it("deve retornar lista de alertas SLA (pode ser vazia)", async () => {
      const res = await request(app).get("/api/service-desk/sla-alerts");
      expect(res.status).toBe(200);
      expect(Array.isArray(res.body)).toBe(true);
    });
  });

  describe("POST /api/service-desk/tickets", () => {
  ...
  describe("GET /api/service-desk/tickets", () => {
    it("deve listar tickets", async () => {
      const res = await request(app).get("/api/service-desk/tickets");
      expect(res.status).toBe(200);
      expect(Array.isArray(res.body)).toBe(true);
    });
    ...
    it("deve filtrar por prioridade", async () => {
      const res = await request(app).get(
        "/api/service-desk/tickets?priority=alta",
      );
      expect(res.status).toBe(200);
      expect(
        res.body.every(
  ...
  describe("GET /api/service-desk/tickets/:id", () => {
    it("deve retornar 404 para ticket inexistente", async () => {
      const res = await request(app).get(
        "/api/service-desk/tickets/tkt-inexistente",
      );
    ...
    it("deve retornar ticket por ID", async () => {
      ...
      const getRes = await request(app).get(`/api/service-desk/tickets/${id}`);
      expect(getRes.status).toBe(200);
      expect(getRes.body.id).toBe(id);
    });
  });

  describe("POST /api/service-desk/tickets/:id/comment", () => {
    it("deve adicionar comentário e mudar status", async () => {
      ...
      const commentRes = await request(app)
        .post(`/api/service-desk/tickets/${id}/comment`)
        .send({
          author: "analista",
  ...
  describe("POST /api/service-desk/tickets/:id/escalate", () => {
    it("deve escalonar ticket de L1 para L2", async () => {
      ...
      const escRes = await request(app)
        .post(`/api/service-desk/tickets/${id}/escalate`)
  ...
  describe("POST /api/service-desk/tickets/:id/close", () => {
    it("deve encerrar ticket", async () => {
      ...
      const closeRes = await request(app)
        .post(`/api/service-desk/tickets/${id}/close`)
