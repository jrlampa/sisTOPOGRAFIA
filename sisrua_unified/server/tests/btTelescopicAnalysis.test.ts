...
describe("calculateLmaxByConductor", () => {
  it("retorna mapa vazio se demanda for zero", () => {
    const params: LmaxParams = {
      demandaAcumuladaKva: 0,
      phase: "TRI",
      temperaturaC: 75,
      quedaDisponivel: 0.05,
    };
...
describe("analyzeTelescopicPaths", () => {
  ...
  it("inclui tensão atual do terminal em cada sugestão", () => {
    const input = makeFailingLinearInput(70);
    const output = calculateBtRadial(input);
    const analysis = analyzeTelescopicPaths(input, output);

    for (const suggestion of analysis.suggestions) {
      expect(typeof suggestion.currentVoltageEndV).toBe("number");
      expect(suggestion.currentVoltageEndV).toBeLessThan(117);
    }
  });
...
describe("analyzeTelescopicPaths – cobertura greedy e helpers", () => {
  ...
  it("greedy telescopico gera pathEdges para terminal reprovado", () => {
    ...
    expect(analysis.suggestions[0].projectedVoltageEndV).toBeGreaterThanOrEqual(
      0,
    );
  });

  it("buildEmptySuggestion mantém regra de upgrade apenas por saturação > 100%", () => {
    ...
    expect(analysis.suggestions[0].requiresTransformerUpgrade).toBe(false);
  });
