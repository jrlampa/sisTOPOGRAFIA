...
describe("RFP Readiness Routes (117)", () => {
  describe("GET /api/rfp/library", () => {
    it("deve retornar biblioteca completa", async () => {
      const res = await request(app).get("/api/rfp/library");
    ...
    it("deve filtrar por categoria seguranca", async () => {
      const res = await request(app).get("/api/rfp/library?category=seguranca");
      expect(res.status).toBe(200);
      expect(Array.isArray(res.body)).toBe(true);
  ...
  describe("GET /api/rfp/search", () => {
    it("deve encontrar questões sobre LGPD", async () => {
      const res = await request(app).get("/api/rfp/search?q=lgpd");
      expect(res.status).toBe(200);
      expect(Array.isArray(res.body)).toBe(true);
      expect(res.body.length).toBeGreaterThanOrEqual(1);
    });
    ...
    it("deve retornar 400 sem parâmetro q", async () => {
      const res = await request(app).get("/api/rfp/search");
      expect(res.status).toBe(400);
    });

    it("deve retornar lista vazia para busca sem resultado", async () => {
      const res = await request(app).get(
        "/api/rfp/search?q=termosemresultado123abc",
      );
      expect(res.status).toBe(200);
  ...
  describe("GET /api/rfp/architecture", () => {
    it("deve retornar referência de arquitetura completa", async () => {
      const res = await request(app).get("/api/rfp/architecture");
      expect(res.status).toBe(200);
      expect(Array.isArray(res.body)).toBe(true);
    ...
    it("deve filtrar por tier backend", async () => {
      const res = await request(app).get("/api/rfp/architecture?tier=backend");
      expect(res.status).toBe(200);
      expect(
  ...
  describe("GET /api/rfp/readiness-profile", () => {
    it("deve retornar perfil de prontidão com cobertura", async () => {
      const res = await request(app).get("/api/rfp/readiness-profile");
      expect(res.status).toBe(200);
