...
const makeLinearInput = (
  overrides: Partial<BtRadialTopologyInput> = {},
...
describe("btRadialCalculationService – validation (E1-H1)", () => {
  ...
  it("rejects edge without conductorId", () => {
    ...
    // @ts-expect-error: intentional bad input
    input.edges = [
      { fromNodeId: "R", toNodeId: "A", conductorId: "", lengthMeters: 50 },
    ];
    expect(() => calculateBtRadial(input)).toThrow(BtRadialValidationError);
  });
...
describe("btRadialCalculationService – qt propagation (E3-H1)", () => {
  ...
  it("qtSegment uses Ω/km with length conversion m->km", () => {
    ...
    const expectedSegment = (1 * 15 * impedance * lengthKm) / 220 ** 2;
...
describe("btRadialCalculationService – worst case (E4-H1)", () => {
  it("cqtGlobal is the maximum qtTotal across terminals", () => {
    const result = calculateBtRadial(makeLinearInput());
    const maxTerminalQt = Math.max(
      ...result.terminalResults.map((t) => t.qtTotal),
    );
  ...
  it("worstTerminalNodeId is the terminal with highest qtTotal", () => {
    const result = calculateBtRadial(makeLinearInput());
    const worst = result.terminalResults.reduce((a, b) =>
      a.qtTotal > b.qtTotal ? a : b,
    );
  ...
  it("tie in qtTotal is resolved deterministically", () => {
    ...
    const r1 = calculateBtRadial(input);
    const r2 = calculateBtRadial(input);
    expect(r1.worstCase.worstTerminalNodeId).toBe(
      r2.worstCase.worstTerminalNodeId,
    );
  });
});
...
describe("btRadialCalculationService – demand accumulation (E5-H1)", () => {
  ...
  it("root node accumulated demand equals totalDemandKva", () => {
    ...
    expect(rootNode.accumulatedDemandKva).toBeCloseTo(
      result.totalDemandKva,
      10,
    );
  });
});
...
describe("btRadialCalculationService – consistency checks (E5-H2)", () => {
  it("valid input generates no error-level alerts", () => {
    const result = calculateBtRadial(makeLinearInput());
    const errorAlerts = result.consistencyAlerts.filter(
      (a) => a.severity === "error",
    );
...
// ─── Consistency alerts: ANEEL regulatory thresholds ─────────────────────────

describe("btRadialCalculationService – consistency alerts (ANEEL PRODIST Módulo 8)", () => {
  function makeHighCqtInput(): BtRadialTopologyInput {
    return {
      transformer: {
        id: "T",
        rootNodeId: "R",
      ...
      nodes: [
        { id: "R", load: { localDemandKva: 0 } },
        { id: "A", load: { localDemandKva: 200 } },
      ],
      edges: [
        {
          fromNodeId: "R",
          toNodeId: "A",
          conductorId: "25 Al - Arm",
          lengthMeters: 5000,
        },
      ],
      phase: "MONO",
  ...
  it("emite CQT_HIGH quando queda de tensão supera 8% (limite ANEEL)", () => {
    const result = calculateBtRadial(makeHighCqtInput());
    const alert = result.consistencyAlerts.find((a) => a.code === "CQT_HIGH");
    expect(alert).toBeDefined();
    expect(alert!.severity).toBe("warn");
  });

  it("não emite CQT_HIGH para rede dentro do limite ANEEL", () => {
    const input: BtRadialTopologyInput = {
      transformer: {
        id: "T",
        rootNodeId: "R",
    ...
    const result = calculateBtRadial(input);
    expect(result.worstCase.cqtGlobal).toBeLessThan(0.08);
    const alert = result.consistencyAlerts.find((a) => a.code === "CQT_HIGH");
    expect(alert).toBeUndefined();
  });

  it("emite TRAFO_OVERLOAD quando demanda supera potência nominal do transformador", () => {
    const input: BtRadialTopologyInput = {
      transformer: {
        id: "T",
        rootNodeId: "R",
    ...
    const result = calculateBtRadial(input);
    const alert = result.consistencyAlerts.find(
      (a) => a.code === "TRAFO_OVERLOAD",
    );
    expect(alert).toBeDefined();
    expect(alert!.severity).toBe("warn");
  });

  it("não emite TRAFO_OVERLOAD quando demanda está abaixo do nominal", () => {
    const input: BtRadialTopologyInput = {
      transformer: {
        id: "T",
        rootNodeId: "R",
    ...
    const result = calculateBtRadial(input);
    expect(
      result.consistencyAlerts.find((a) => a.code === "TRAFO_OVERLOAD"),
    ).toBeUndefined();
  });

  it("emite VOLTAGE_LOW quando tensão no terminal fica abaixo de 116,84 V (127 × 0,92)", () => {
    const result = calculateBtRadial(makeHighCqtInput());
    const alert = result.consistencyAlerts.find(
      (a) => a.code === "VOLTAGE_LOW",
    );
    expect(alert).toBeDefined();
    expect(alert!.severity).toBe("warn");
  });
...
describe("btRadialCalculationService – workbook parity (ESQ ATUAL baseline)", () => {
  ...
  it("qtTrafo tolerance within 1e-4 relative (workbook parity gate)", () => {
    ...
    const result = calculateBtRadial(input);
    const relDiff =
      Math.abs(result.qtTrafo - EXPECTED_QT_MTTR) / EXPECTED_QT_MTTR;
