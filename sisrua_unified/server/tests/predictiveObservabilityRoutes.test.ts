...
describe("Predictive Observability Routes (18)", () => {
  ...
  describe("POST /api/observability/metrics", () => {
    it("deve ingerir métrica válida", async () => {
      const res = await request(app).post("/api/observability/metrics").send({
  ...
  describe("GET /api/observability/metrics/:metric/series", () => {
    it("deve retornar série temporal da métrica", async () => {
      await request(app).post("/api/observability/metrics").send({
        metric: "queue_depth",
        value: 12,
      });
      await request(app).post("/api/observability/metrics").send({
      ...
      const res = await request(app).get(
        "/api/observability/metrics/queue_depth/series?limit=2",
      );
      expect(res.status).toBe(200);
    ...
    it("deve retornar 400 para métrica inválida", async () => {
      const res = await request(app).get(
        "/api/observability/metrics/nao_existe/series",
      );
      expect(res.status).toBe(400);
    });
  });

  describe("GET /api/observability/metrics/:metric/stats", () => {
    it("deve retornar estatísticas com p95 e p99", async () => {
      ...
      const res = await request(app).get(
        "/api/observability/metrics/bt_calculation_seconds/stats?windowMinutes=1440",
      );
      expect(res.status).toBe(200);
  ...
  describe("GET /api/observability/metrics/:metric/anomalies", () => {
    it("deve detectar anomalias por z-score", async () => {
      ...
      const res = await request(app).get(
        "/api/observability/metrics/api_latency_ms/anomalies?windowMinutes=1440&zThreshold=2.5",
      );
      expect(res.status).toBe(200);
      expect(Array.isArray(res.body)).toBe(true);
      expect(res.body.length).toBeGreaterThanOrEqual(1);
  ...
  describe("GET /api/observability/metrics/:metric/signal", () => {
    it("deve retornar sinal preditivo com tendência e risco", async () => {
      ...
      const res = await request(app).get(
        "/api/observability/metrics/error_rate_pct/signal?windowMinutes=360",
      );
      expect(res.status).toBe(200);
  ...
  describe("GET /api/observability/overview", () => {
    it("deve retornar visão consolidada de observabilidade", async () => {
      const res = await request(app).get("/api/observability/overview");
      expect(res.status).toBe(200);
