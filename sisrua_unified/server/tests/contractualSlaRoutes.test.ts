...
describe("Contractual SLA Routes (114)", () => {
  describe("GET /api/contractual-sla/catalog", () => {
    it("deve retornar catálogo de SLAs com 8 fluxos", async () => {
      const res = await request(app).get("/api/contractual-sla/catalog");
  ...
  describe("GET /api/contractual-sla/flows/:flowId", () => {
    it("deve retornar SLA de exportacao_dxf", async () => {
      const res = await request(app).get("/api/contractual-sla/flows/exportacao_dxf");
      expect(res.status).toBe(200);
    ...
    it("deve retornar 404 para fluxo inexistente", async () => {
      const res = await request(app).get("/api/contractual-sla/flows/fluxo_inexistente");
      expect(res.status).toBe(404);
    });
  });

  describe("POST /api/contractual-sla/events", () => {
    it("deve registrar evento de sucesso", async () => {
      const res = await request(app).post("/api/contractual-sla/events").send({
    ...
    it("deve retornar 400 para payload inválido", async () => {
      const res = await request(app)
        .post("/api/contractual-sla/events")
        .send({ flowId: "invalido", outcome: "success", durationMs: -1 });
      expect(res.status).toBe(400);
    });
  });

  describe("GET /api/contractual-sla/flows/:flowId/compliance", () => {
    it("deve retornar relatório de conformidade do fluxo", async () => {
      const res = await request(app).get(
        "/api/contractual-sla/flows/exportacao_dxf/compliance",
      );
      expect(res.status).toBe(200);
  ...
  describe("GET /api/contractual-sla/compliance", () => {
    it("deve retornar relatório de todos os fluxos", async () => {
      const res = await request(app).get("/api/contractual-sla/compliance");
      expect(res.status).toBe(200);
      expect(Array.isArray(res.body)).toBe(true);
      expect(res.body.length).toBe(8);
    });
  ...
  describe("GET /api/contractual-sla/violations", () => {
    it("deve retornar lista de violações (pode ser vazia sem eventos de falha)", async () => {
      const res = await request(app).get("/api/contractual-sla/violations");
      expect(res.status).toBe(200);
      expect(Array.isArray(res.body)).toBe(true);
    });
  });

  describe("GET /api/contractual-sla/flows/:flowId/events", () => {
    it("deve retornar eventos registrados do fluxo", async () => {
      const res = await request(app).get(
        "/api/contractual-sla/flows/exportacao_dxf/events",
      );
      expect(res.status).toBe(200);
      expect(Array.isArray(res.body)).toBe(true);
    });
