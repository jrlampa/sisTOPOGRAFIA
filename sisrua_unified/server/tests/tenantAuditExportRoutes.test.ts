...
describe("Tenant Audit Export Routes (34)", () => {
  ...
  it("POST /api/tenant-audit/logs — deve ingestar evento", async () => {
  ...
  it("POST /api/tenant-audit/logs — 400 para payload inválido", async () => {
    const res = await request(app)
      .post("/api/tenant-audit/logs")
  ...
  it("GET /api/tenant-audit/logs/:tenantId — deve consultar eventos", async () => {
    await request(app).post("/api/tenant-audit/logs").send({
      tenantId, tipo: "operacao", actor: "admin", recurso: "/dxf", acao: "POST", resultado: "sucesso",
    });
    const res = await request(app).get(`/api/tenant-audit/logs/${tenantId}`);
    expect(res.status).toBe(200);
    expect(res.body.total).toBe(1);
    expect(Array.isArray(res.body.eventos)).toBe(true);
  });

  it("POST /api/tenant-audit/export — deve exportar JSON com hash", async () => {
    await request(app).post("/api/tenant-audit/logs").send({
      tenantId, tipo: "acesso", actor: "user", recurso: "/api/jobs", acao: "GET", resultado: "sucesso",
    });
    const res = await request(app)
      .post("/api/tenant-audit/export")
  ...
  it("POST /api/tenant-audit/export — deve exportar CSV", async () => {
    await request(app).post("/api/tenant-audit/logs").send({
  ...
  it("GET /api/tenant-audit/stats/:tenantId — deve retornar estatísticas", async () => {
    await request(app).post("/api/tenant-audit/logs").send({
      tenantId, tipo: "admin", actor: "admin", recurso: "/admin", acao: "DELETE", resultado: "negado",
    });
    const res = await request(app).get(`/api/tenant-audit/stats/${tenantId}`);
    expect(res.status).toBe(200);
