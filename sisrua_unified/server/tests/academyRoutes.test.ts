...
const asWriter = (req: ReturnType<typeof request>) => req.set('x-user-id', 'academy-writer');
...
describe('POST /trilhas', () => {
  ...
  it('cria trilha com status rascunho', async () => {
    const res = await asWriter(request(app).post(`${BASE}/trilhas`)).send(trilhaPayload);
  ...
  it('rejeita sem titulo', async () => {
    const res = await asWriter(request(app).post(`${BASE}/trilhas`)).send({
      ...trilhaPayload,
      titulo: '',
    });
...
describe('GET /trilhas', () => {
  ...
  it('filtra por tenantId', async () => {
    await asWriter(request(app).post(`${BASE}/trilhas`)).send({ ...trilhaPayload, tenantId: 'tA' });
    await asWriter(request(app).post(`${BASE}/trilhas`)).send({ ...trilhaPayload, tenantId: 'tB' });
...
describe('GET /trilhas/:id', () => {
  ...
  it('retorna trilha existente', async () => {
    await asWriter(request(app).post(`${BASE}/trilhas`)).send(trilhaPayload);
...
describe('POST /trilhas/:id/cursos', () => {
  it('adiciona curso à trilha', async () => {
    await asWriter(request(app).post(`${BASE}/trilhas`)).send(trilhaPayload);
    const res = await asWriter(request(app).post(`${BASE}/trilhas/tr-1/cursos`)).send(cursoPayload);
    expect(res.status).toBe(201);
  ...
  it('retorna 422 para trilha inexistente', async () => {
    const res = await asWriter(request(app).post(`${BASE}/trilhas/tr-999/cursos`)).send(
      cursoPayload
    );
...
describe('POST /cursos/:id/modulos', () => {
  it('adiciona módulo ao curso', async () => {
    await asWriter(request(app).post(`${BASE}/trilhas`)).send(trilhaPayload);
    await asWriter(request(app).post(`${BASE}/trilhas/tr-1/cursos`)).send(cursoPayload);
    const res = await asWriter(request(app).post(`${BASE}/cursos/cu-1/modulos`)).send(
      moduloPayload
    );
    expect(res.status).toBe(201);
  ...
  it('retorna 422 para curso inexistente', async () => {
    const res = await asWriter(request(app).post(`${BASE}/cursos/cu-999/modulos`)).send(
      moduloPayload
    );
    expect(res.status).toBe(422);
  });
...
describe('Fluxo completo de progresso e certificação', () => {
  ...
  beforeEach(async () => {
    const t = await asWriter(request(app).post(`${BASE}/trilhas`)).send(trilhaPayload);
    trilhaId = t.body.id;
    const c = await asWriter(request(app).post(`${BASE}/trilhas/${trilhaId}/cursos`)).send(
      cursoPayload
    );
    cursoId = c.body.id;
    const m = await asWriter(request(app).post(`${BASE}/cursos/${cursoId}/modulos`)).send(
      moduloPayload
    );
    moduloId = m.body.id;
    // Publica trilha para permitir progresso
    await asWriter(request(app).post(`${BASE}/trilhas/${trilhaId}/publicar`));
    const pg = await asWriter(request(app).post(`${BASE}/progresso`)).send({
      usuarioId: 'aluno-1',
      trilhaId,
    });
  ...
  it('emite certificado após 100% de conclusão', async () => {
    await request(app)
      ...
      .set('x-user-id', 'academy-writer')
      .send({ moduloId });
    const res = await asWriter(request(app).post(`${BASE}/progresso/${progressoId}/certificar`));
    expect(res.status).toBe(200);
