...
const makeTopology = (overrides: Partial<Parameters<typeof computeBtDerivedState>[0]> = {}) => ({
    poles: [
...
const withClandestinoConstants = () => {
    getSyncMock.mockImplementation((_ns: unknown, key: unknown) => {
        ...
        if (key === 'CLIENT_TO_DIVERSIF_FACTOR') return { '1': 1.0, '2': 0.9, '3': 0.85, '5': 0.8 };
        return null;
    });
};
...
describe('computeBtDerivedState – accumulatedByPole (ramais)', () => {
    ...
    it('returns sorted list with most-loaded pole first', () => {
        ...
        for (let i = 1; i < result.accumulatedByPole.length; i++) {
            expect(result.accumulatedByPole[i - 1].accumulatedDemandKva)
                .toBeGreaterThanOrEqual(result.accumulatedByPole[i].accumulatedDemandKva);
        }
    });
});
...
describe('computeBtDerivedState – estimatedByTransformer', () => {
    ...
    it('uses measured demandKw for transformers with readings', () => {
        const topology = makeTopology();
        const result = computeBtDerivedState(topology, 'geral', 0);
        const entry = result.estimatedByTransformer.find((e) => e.transformerId === 'tr1');
    ...
    it('returns zero estimated demand for transformer without poleId link', () => {
        const topology = {
            poles: [{ id: 'p1', lat: 0, lng: 0, ramais: [{ quantity: 2, ramalType: 'Ramal BT' }] }],
            transformers: [
                { id: 'tr1', demandKw: 10, readings: [] },
            ],
            edges: [],
        };
        const result = computeBtDerivedState(topology, 'geral', 0);
    ...
    it('assigns each pole to its closest transformer', () => {
        const topology = {
            poles: [
                { id: 'p1', lat: 0, lng: 0, ramais: [{ quantity: 5, ramalType: 'Ramal BT' }] },
                { id: 'p2', lat: 0.001, lng: 0, ramais: [{ quantity: 3, ramalType: 'Ramal BT' }] },
                { id: 'p3', lat: 0.002, lng: 0, ramais: [{ quantity: 4, ramalType: 'Ramal BT' }] },
            ],
            transformers: [
        ...
        const result = computeBtDerivedState(topology, 'geral', 0);
        const tr1 = result.estimatedByTransformer.find((e) => e.transformerId === 'tr1');
        const tr2 = result.estimatedByTransformer.find((e) => e.transformerId === 'tr2');
        ...
        const result = computeBtDerivedState(
            {
                ...
                transformers: [{ id: 'tr1', poleId: 'p1', demandKw: 10, readings: [{ id: 'r1' }] }],
                edges: [],
            },
            'geral',
            0,
