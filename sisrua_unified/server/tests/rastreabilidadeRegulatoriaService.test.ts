...
describe('rastreabilidadeRegulatoriaService', () => {
  ...
  describe('adicionarItem()', () => {
    it('adiciona item extra e o torna visível via listarItens', () => {
      const novoItem: Omit<ItemRastreabilidade, 'id' | 'verificadoEm'> = {
        ...
        implementacoes: [{ tipo: 'servico', referencia: 'server/services/mtCalculationService.ts', descricao: 'Cálculo MT' }],
        testes: [{ tipo: 'teste', referencia: 'server/tests/mtCalculationService.test.ts', descricao: 'Testes MT' }],
        status: 'parcial',
      };
  ...
  describe('atualizarStatus()', () => {
    ...
    it('atualiza status de item extra', () => {
      ...
      const ok = atualizarStatus(adicionado.id, 'conforme', 'verificado manualmente');
      expect(ok).toBe(true);
      expect(obterItem(adicionado.id)!.status).toBe('conforme');
      expect(obterItem(adicionado.id)!.observacao).toBe('verificado manualmente');
    });
  ...
  describe('gerarRelatorio()', () => {
    ...
    it('soma de conformes + parciais + naoConformes + naoAvaliados == totalItens', () => {
      const rel = gerarRelatorio();
      expect(rel.conformes + rel.parciais + rel.naoConformes + rel.naoAvaliados).toBe(rel.totalItens);
    });
