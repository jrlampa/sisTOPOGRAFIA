...
export class ElevationService {
  ...
  static calculateDistance(start: GeoLocation, end: GeoLocation): number {
    return haversineDistanceMeters(start, end);
  }
  ...
  static async getElevationProfile(
    ...
    const useTopodata =
      TopodataService.isWithinBrazil(start.lat, start.lng) &&
   ...
   */
  private static async getTopodataProfile(
    start: GeoLocation,
    end: GeoLocation,
    steps: number,
  ) {
    try {
      ...
      logger.warn("TOPODATA failed, falling back to open-elevation", {
        error: error.message,
      });
      return this.getOpenElevationProfile(start, end, steps);
    }
   ...
   */
  private static async getOpenElevationProfile(
    start: GeoLocation,
    end: GeoLocation,
    steps: number,
  ) {
  ...
  static async getElevationAt(
    ...
    if (TopodataService.isWithinBrazil(lat, lng)) {
      try {
        ...
        logger.warn("TOPODATA single point failed, trying open-elevation", {
          error: error.message,
        });
