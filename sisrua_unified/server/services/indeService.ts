...
export class IndeService {
     ...
     */
    static async getWfsCapabilities(source: keyof typeof INDE_ENDPOINTS = 'ibge'): Promise<WfsFeatureType[]> {
    ...
    static async getFeaturesByBBox(
        ...
        try {
            ...
            logger.error('INDE WFS features fetch failed', { error, layer: layerName });
            return null;
        }
    }
     ...
     */
    static async getCommonLayers(source: keyof typeof INDE_ENDPOINTS = 'ibge'): Promise<WfsFeatureType[]> {
        const allCapabilities = await this.getWfsCapabilities(source);
        
        // Filter for commonly useful layers
        const keywords = [
            'municipio', 'municipal', 'limite',
            'rodovia', 'rodovias', 'estrada',
            'hidrografia', 'rio', 'curso',
            'vegetacao', 'floresta',
            'assentamento', 'rural'
        ];
