    ...
    public async runMaintenance(): Promise<MaintenanceStats> {
        ...
        if (sql) {
            try {
                ...
                logger.info(`[Maintenance] DB Cleanup complete: ${stats.auditLogsDeleted} logs, ${stats.jobsDeleted} jobs deleted.`);
            } catch (error) {
                logger.error('[Maintenance] DB Cleanup failed', { error: error instanceof Error ? error.message : String(error) });
            }
        }
        ...
        try {
            ...
            logger.error('[Maintenance] File cleanup failed', { error: error instanceof Error ? error.message : String(error) });
        }
