...
export class IbgeService {
    ...
    static async getStates(): Promise<IbgeLocation[]> {
        ...
        try {
            const response = await fetch(`${IBGE_BASE_URL}/localidades/estados?orderBy=nome`);
     ...
     */
    static async getMunicipalityBoundary(municipioId: string): Promise<any | null> {
        ...
        try {
            ...
            logger.error('IBGE getMunicipalityBoundary failed', { error, municipioId });
            return null;
        }
    }
    ...
    static async findMunicipioByName(name: string, uf?: string): Promise<IbgeMunicipio | null> {
        try {
            ...
            const statesToSearch = uf ? [uf] : (await this.getStates())
                .map(s => s.sigla)
            ...
            for (const state of statesToSearch) {
                ...
                const found = municipios.find(m => {
                    const normalizedMunicipio = m.nome.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
                    return normalizedMunicipio === normalizedName || 
     ...
     */
    static async findMunicipioByCoordinates(lat: number, lng: number): Promise<LocationInfo | null> {
        try {
            ...
            const data = await response.json();
            if (data && data.nome) {
                const municipio = await this.findMunicipioByName(data.nome, data.estado);
                if (municipio) {
                    return {
     ...
     */
    private static async fallbackReverseGeocode(lat: number, lng: number): Promise<LocationInfo | null> {
     ...
     */
    private static identifyStateByCoordinates(lat: number, lng: number): { sigla: string; nome: string; regiao: string } | null {
        // Coordinate ranges for Brazilian states (simplified)
        const stateRanges: Record<string, { lat: [number, number]; lng: [number, number]; nome: string; regiao: string }> = {
            'SP': { lat: [-25.3, -19.8], lng: [-53.1, -44.3], nome: 'São Paulo', regiao: 'Sudeste' },
            'RJ': { lat: [-23.0, -20.8], lng: [-44.9, -40.9], nome: 'Rio de Janeiro', regiao: 'Sudeste' },
            'MG': { lat: [-22.9, -14.0], lng: [-51.5, -39.8], nome: 'Minas Gerais', regiao: 'Sudeste' },
            'ES': { lat: [-21.3, -18.0], lng: [-41.9, -39.7], nome: 'Espírito Santo', regiao: 'Sudeste' },
            'PR': { lat: [-26.7, -22.5], lng: [-54.6, -48.0], nome: 'Paraná', regiao: 'Sul' },
            'SC': { lat: [-29.3, -25.9], lng: [-53.8, -48.3], nome: 'Santa Catarina', regiao: 'Sul' },
            'RS': { lat: [-33.7, -27.0], lng: [-57.6, -49.7], nome: 'Rio Grande do Sul', regiao: 'Sul' },
            'MS': { lat: [-24.1, -17.8], lng: [-58.4, -50.9], nome: 'Mato Grosso do Sul', regiao: 'Centro-Oeste' },
            'MT': { lat: [-18.0, -9.0], lng: [-61.6, -50.0], nome: 'Mato Grosso', regiao: 'Centro-Oeste' },
            'GO': { lat: [-19.5, -12.3], lng: [-53.3, -45.9], nome: 'Goiás', regiao: 'Centro-Oeste' },
            'DF': { lat: [-16.1, -15.5], lng: [-48.3, -47.2], nome: 'Distrito Federal', regiao: 'Centro-Oeste' },
            'BA': { lat: [-18.3, -8.5], lng: [-46.6, -37.3], nome: 'Bahia', regiao: 'Nordeste' },
            'SE': { lat: [-11.6, -9.5], lng: [-38.2, -36.3], nome: 'Sergipe', regiao: 'Nordeste' },
            'AL': { lat: [-10.5, -8.8], lng: [-38.0, -35.1], nome: 'Alagoas', regiao: 'Nordeste' },
            'PE': { lat: [-9.5, -7.2], lng: [-41.4, -34.8], nome: 'Pernambuco', regiao: 'Nordeste' },
            'PB': { lat: [-8.3, -6.0], lng: [-38.8, -34.8], nome: 'Paraíba', regiao: 'Nordeste' },
            'RN': { lat: [-6.8, -4.8], lng: [-38.6, -35.1], nome: 'Rio Grande do Norte', regiao: 'Nordeste' },
            'CE': { lat: [-7.9, -2.8], lng: [-41.4, -37.3], nome: 'Ceará', regiao: 'Nordeste' },
            'PI': { lat: [-10.9, -2.8], lng: [-45.2, -40.5], nome: 'Piauí', regiao: 'Nordeste' },
            'MA': { lat: [-10.3, 2.7], lng: [-47.5, -41.3], nome: 'Maranhão', regiao: 'Nordeste' },
            'PA': { lat: [-9.8, 4.4], lng: [-58.0, -46.7], nome: 'Pará', regiao: 'Norte' },
            'AP': { lat: [0.0, 5.3], lng: [-54.8, -49.8], nome: 'Amapá', regiao: 'Norte' },
            'RR': { lat: [0.0, 5.3], lng: [-64.8, -58.9], nome: 'Roraima', regiao: 'Norte' },
            'AM': { lat: [-9.7, 5.3], lng: [-73.8, -56.0], nome: 'Amazonas', regiao: 'Norte' },
            'AC': { lat: [-11.5, -7.0], lng: [-74.0, -66.5], nome: 'Acre', regiao: 'Norte' },
            'RO': { lat: [-13.7, -8.0], lng: [-66.8, -59.4], nome: 'Rondônia', regiao: 'Norte' },
            'TO': { lat: [-13.7, -5.1], lng: [-50.9, -45.7], nome: 'Tocantins', regiao: 'Norte' },
        };
