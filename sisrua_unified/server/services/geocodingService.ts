...
export class GeocodingService {
  ...
  private static parseUtm(query: string): {
    ...
    const utmMatch = normalized.match(
      /(\d{1,2})([C-HJ-NP-X]|[NS])?\s+(\d{6,7}(?:\.\d+)?)\s+(\d{7}(?:\.\d+)?)/i,
    );
  ...
  static utmToLatLon(
    ...
    const latRad =
      ...
      ((n1 * Math.tan(fp)) / r1) *
        (Math.pow(d, 2) / 2 -
          ((5 + 3 * t1 + 10 * c1 - 4 * Math.pow(c1, 2) - 9 * e2) *
            Math.pow(d, 4)) /
            24 +
          ((61 +
            90 * t1 +
  ...
  static async resolveLocation(query: string): Promise<GeoLocation | null> {
    ...
    if (utm) {
      ...
      if (coords) {
        return {
          ...coords,
          label: `UTM ${utm.zone}${utm.hemisphere} ${utm.easting} ${utm.northing}`,
        };
      }
    }
   ...
   */
  private static async searchIbgeMunicipio(
    query: string,
  ): Promise<GeoLocation | null> {
    ...
    const municipio = await IbgeService.findMunicipioByName(
      municipioName,
      ufHint,
    );
    ...
    if (municipio) {
      ...
      const hasRelevantToken = queryTokens.some((token) =>
        normalizedMunicipio.includes(token),
      );
      if (!hasRelevantToken) {
        return null;
      }
   ...
   */
  private static getStateCentroid(
    uf: string,
