...
function salvaguardasPorCategorias(categorias: CategoriaDado[]): string[] {
  ...
  if (
    categorias.includes("sensivel_saude") ||
    categorias.includes("sensivel_biometrico")
  ) {
    salvaguardas.push(
      "Tratamento de dados sensíveis restrito a pessoal autorizado (Art. 11)",
    );
    salvaguardas.push(
  ...
  if (categorias.includes("localizacao")) {
    salvaguardas.push(
      "Minimização de dados: coordenadas agregadas em vez de ponto exato quando possível",
    );
  }
...
function riscosPorFluxo(fluxo: FluxoTratamento): string[] {
  ...
  if (fluxo.compartilhaTerceiros) {
    riscos.push(
      "Compartilhamento com terceiros: risco de acesso não autorizado — mitigado por DPA (Data Processing Agreement)",
    );
  }
  if (fluxo.transferenciaInternacional) {
    riscos.push(
      "Transferência internacional: verificar adequação do país destino (Art. 33 LGPD)",
    );
  }
  if (
    fluxo.categorias.includes("sensivel_saude") ||
    fluxo.categorias.includes("sensivel_biometrico")
  ) {
    riscos.push(
      "Dados sensíveis: risco de discriminação — minimização e base legal específica obrigatórias",
    );
  }
  if (fluxo.retencaoDias > 1825) {
    // > 5 anos
    riscos.push(
      `Retenção longa (${fluxo.retencaoDias} dias): justificar necessidade conforme princípio da necessidade (Art. 6º III)`,
    );
  }
  if (riscos.length === 0) {
...
export function registrarFluxo(
  input: Omit<
    FluxoTratamento,
    "id" | "artigoLgpd" | "registradoEm" | "atualizadoEm"
  >,
): FluxoTratamento {
  const id = randomUUID();
  const agora = isoNow();
...
export function atualizarSolicitacao(
  ...
  const updated: SolicitacaoDireito = {
    ...
    resposta,
    atendidaEm:
      status === "atendida" || status === "indeferida"
        ? isoNow()
        : sol.atendidaEm,
  };
...
export function listarSolicitacoesPorTitular(
  titularId: string,
): SolicitacaoDireito[] {
  return Array.from(solicitacoesMap.values()).filter(
    (s) => s.titularId === titularId,
  );
}
