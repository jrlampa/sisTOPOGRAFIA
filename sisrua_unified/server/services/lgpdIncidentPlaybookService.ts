...
function gerarAcoes(severidade: SeveridadeIncidente): AcaoPlaybook[] {
  const prazoContencao =
    severidade === "critica" ? 2 : severidade === "alta" ? 4 : 8;
  return [
    {
      etapa: "deteccao_triagem",
      descricao:
        "Confirmar o incidente, classificar tipo e severidade, acionar CISO/DPO.",
    ...
    },
    {
      ...
      descricao:
        "Isolar sistemas afetados, revogar acessos comprometidos, preservar evidências forenses.",
    ...
    },
    {
      ...
      descricao:
        "Quantificar titulares afetados, identificar categorias de dados, avaliar danos potenciais.",
    ...
    },
    {
      ...
      descricao:
        `Notificar ANPD via portal gov.br (prazo: ${PRAZO_ANPD_HORAS}h da ciência — Art. 48 LGPD + Res. CD/ANPD nº 4/2023). ` +
    ...
    },
    {
      ...
      descricao:
        "Comunicar titulares afetados com linguagem clara sobre o incidente, dados envolvidos e medidas de proteção adotadas.",
    ...
    },
    {
      ...
      descricao:
        "Corrigir vulnerabilidade raiz, atualizar controles, documentar lições aprendidas, revisar RIPD do fluxo afetado.",
...
export function concluirEtapa(
  ...
  const updated: IncidenteLgpd = {
    ...
    acoes,
    status,
    notificacaoAnpdEnviada:
      etapa === "notificacao_anpd" ? true : inc.notificacaoAnpdEnviada,
...
export function listarIncidentesAbertos(): IncidenteLgpd[] {
  return Array.from(incidentesMap.values()).filter(
...
export function resumoIncidentes(): ResumoIncidente[] {
  return Array.from(incidentesMap.values()).map((i) => ({
    ...
    dentroDosPrazo: i.notificacaoAnpdEnviada
...
export function verificarPrazosVencidos(): ResumoIncidente[] {
  ...
  return resumoIncidentes().filter(
    (r) =>
      !r.notificacaoAnpdEnviada && agora > new Date(r.prazoAnpdISO).getTime(),
  );
}
