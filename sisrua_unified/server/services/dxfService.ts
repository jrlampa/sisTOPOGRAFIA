...
const createElevationResolver = (terrain: TerrainGrid | undefined, centerElev: number) => {
  if (!terrain || terrain.length === 0 || terrain[0].length === 0) {
    return (_lat: number, _lng: number) => 0;
  }

  const rows = terrain.length;
  const cols = terrain[0].length;
  const latStart = terrain[0][0].lat;
  const lngStart = terrain[0][0].lng;
  const latEnd = terrain[rows - 1][0].lat;
  const lngEnd = terrain[0][cols - 1].lng;
  const latStep = rows > 1 ? (latEnd - latStart) / (rows - 1) : 0;
  const lngStep = cols > 1 ? (lngEnd - lngStart) / (cols - 1) : 0;
  const cache = new Map<string, number>();

  const clamp = (value: number, min: number, max: number) => Math.max(min, Math.min(max, value));

  const nearestElevation = (lat: number, lng: number): number => {
    const roughRow = latStep !== 0 ? Math.round((lat - latStart) / latStep) : 0;
    const roughCol = lngStep !== 0 ? Math.round((lng - lngStart) / lngStep) : 0;

    let closestDist = Infinity;
    let elevation = terrain[0][0].elevation;

    // Probe a 3x3 neighborhood around the estimated grid index when possible.
    for (let r = clamp(roughRow - 1, 0, rows - 1); r <= clamp(roughRow + 1, 0, rows - 1); r++) {
      const row = terrain[r];
      for (let c = clamp(roughCol - 1, 0, row.length - 1); c <= clamp(roughCol + 1, 0, row.length - 1); c++) {
        const p = row[c];
    ...
    // Fallback when step estimates are degenerate/irregular.
  ...
  return (lat: number, lng: number): number => {
    ...
    const normalized = nearestElevation(lat, lng) - centerElev;
...
const generatePolyline = (points: any[], layer: string, isClosed: boolean, height: number, center: GeoLocation, getElevationAt: (lat: number, lng: number) => number, layerOffset: number = 0, linetype: string = 'CONTINUOUS', simplify: boolean = false) => {
  ...
  const zBase = getElevationAt(firstPt.lat, firstPt.lon) + layerOffset;
...
export const generateDXF = (
  ...
  const getElevationAt = createElevationResolver(terrain, centerElev);
  ...
  elements.forEach(el => {
    if (el.type === 'way' && el.geometry) {
      ...
      body += generatePolyline(el.geometry, layer, isClosed, height, center, getElevationAt, layerOffset, linetype, options.simplify);
    }
      ...
      el.members.forEach((member: any) => {
        if (member.type === 'way' && member.role === 'outer' && member.geometry) {
          body += generatePolyline(member.geometry, LAYERS.BUILDINGS.name, true, height, center, getElevationAt, layerOffset, 'CONTINUOUS', options.simplify);
        }
      });
    }
      ...
      if (isInteresting) {
        ...
        const zBase = getElevationAt(el.lat, el.lon);
        body += `0\nCIRCLE\n8\n${layer}\n10\n${x.toFixed(4)}\n20\n${y.toFixed(4)}\n30\n${zBase.toFixed(2)}\n40\n${radius}\n`;
        if (height > 0) body += `39\n${height}\n`;
      }
    }
