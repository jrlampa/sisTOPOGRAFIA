...
const createElevationResolver = (terrain: TerrainGrid | undefined, centerElev: number) => {
  ...
  const latMin = terrain[0][0].lat;
  const lngMin = terrain[0][0].lng;
  const latMax = terrain[rows - 1][cols - 1].lat;
  const lngMax = terrain[rows - 1][cols - 1].lng;
  ...
  return (lat: number, lng: number): number => {
    ...
    // Fast O(1) grid index estimation
    let elevation = 0;
    if (latRange !== 0 && lngRange !== 0) {
        const r = Math.min(rows - 1, Math.max(0, Math.round(((lat - latMin) / latRange) * (rows - 1))));
        const c = Math.min(cols - 1, Math.max(0, Math.round(((lng - lngMin) / lngRange) * (cols - 1))));
        elevation = terrain[r][c].elevation;
    } else {
        elevation = terrain[0][0].elevation;
    }

    const normalized = elevation - centerElev;
...
export const generateDXF = (
  ...
  elements.forEach(el => {
    if (el.type === 'way' && el.geometry) {
      ...
      buffer.push(generatePolyline(el.geometry, layer, isClosed, height, center, getElevationAt, layerOffset, linetype, options.simplify));
    }
