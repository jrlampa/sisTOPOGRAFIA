...
} from "./btRadialCalculationService.js";
  ...
  input: {
    ...
    nodes: [
      { id: "R", load: { localDemandKva: 0 } },
      {
        ...
        load: {
          localDemandKva: 2,
          ramal: { conductorId: "16 Al_CONC_Tri", lengthMeters: 20 },
        },
      },
    ],
    edges: [
...
const SCENARIO_REV0_DB_INDICATORS: BtParityScenario = {
  ...
  description:
    "REV0 workbook – DB K10 (QT_MTTR) parity with same transformer inputs",
  priority: "P0",
  input: {
    transformer: {
      id: "TRAFO_225",
    ...
    nodes: [
      ...
      {
        id: "TOTAL",
        load: { localDemandKva: CQT_REV0_BASELINE_TARGETS.db.k7DemAtual },
      },
    ],
    edges: [
      {
...
const SCENARIO_REV0_LINEAR: BtParityScenario = {
  id: "REV0_LINEAR",
  description:
    "REV0 workbook – linear 3-node topology with REV0 transformer inputs",
  priority: "P0",
  input: {
    transformer: {
      id: "TR225_REV0",
  ...
  expected: {
    ...
    qtTrafo:
      CQT_REV0_BASELINE_TARGETS.db.qtMt +
      (100 / CQT_REV0_BASELINE_TARGETS.db.k6TrAtual) *
        CQT_REV0_BASELINE_TARGETS.db.zPercent,
  },
};
...
const SCENARIO_REV0_IDEMPOTENCY: BtParityScenario = {
  id: "REV0_IDEMPOTENCY",
  description:
    "REV0 workbook – idempotency: two runs produce identical CQT global",
  priority: "P0",
...
function runScenario(scenario: BtParityScenario): BtParityScenarioResult {
  try {
    ...
    for (const [metricName, expectedValue] of Object.entries(
      scenario.expected,
    )) {
      ...
      metrics.push(
        compareMetric(metricName, expectedValue, actualValue, TOLERANCE),
      );
    }

    // Idempotency check
    if (
      (scenario.id === "IDEMPOTENCY" || scenario.id === "REV0_IDEMPOTENCY") &&
      actual2
    ) {
      metrics.push(
        compareMetric(
          "idempotency_cqtGlobal",
          result.worstCase.cqtGlobal,
          actual2.worstCase.cqtGlobal,
          0,
        ),
        compareMetric(
          "idempotency_totalDemand",
          result.totalDemandKva,
          actual2.totalDemandKva,
          0,
        ),
      );
    }
...
export function runBtParitySuite(): BtParitySuiteReport {
  ...
  const totals = {
    ...
    p0Pass: scenarios.filter((s) => s.priority === "P0" && s.status === "pass")
      .length,
    p0Fail: scenarios.filter((s) => s.priority === "P0" && s.status === "fail")
      .length,
    p1Pass: scenarios.filter((s) => s.priority === "P1" && s.status === "pass")
      .length,
    p1Fail: scenarios.filter((s) => s.priority === "P1" && s.status === "fail")
      .length,
    p2Pass: scenarios.filter((s) => s.priority === "P2" && s.status === "pass")
      .length,
    p2Fail: scenarios.filter((s) => s.priority === "P2" && s.status === "fail")
      .length,
  };
 ...
 */
export function runBtParityByPriority(
  priority: BtParityPriority,
): BtParityScenarioResult[] {
 ...
 */
export function listBtParityScenarios(): Array<{
  id: string;
  description: string;
  priority: BtParityPriority;
}> {
