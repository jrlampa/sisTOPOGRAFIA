import {
  ...
  calculateQtPonto,
  DEFAULT_CQT_POWER_FACTOR,
  DEFAULT_QT_PONTO_CALCULATION_METHOD,
  evaluateProtection,
...
interface CqtComputationInputs {
  ...
  qtPontoCalculationMethod?: QtPontoCalculationMethod;
  powerFactor?: number;
  dmdi?: {
 ...
 */
export const attachCqtSnapshotToBtContext = (
  btContext: unknown,
): UnknownRecord | null => {
  ...
  const qtPontoCalculationMethod =
    inputs.qtPontoCalculationMethod ?? DEFAULT_QT_PONTO_CALCULATION_METHOD;
  const powerFactor = inputs.powerFactor ?? DEFAULT_CQT_POWER_FACTOR;
  ...
  if (inputs.branches && inputs.branches.length > 0) {
    ...
    snapshot.branches = {
      ...
      verificarCount: calculatedBranches.filter(
        (item) => item.status === "VERIFICAR",
      ).length,
    };
  ...
  const hasComputedSection = Boolean(
    snapshot.dmdi || snapshot.geral || snapshot.db || snapshot.branches,
  );
  ...
  snapshot.parity = buildCqtParityReport(
    (inputs.scenario ?? "atual") as "atual" | "proj1" | "proj2",
    {
      ...
      geral: snapshot.geral as
        | { p31CqtNoPonto?: number; p32CqtNoPonto?: number }
        | undefined,
      db: snapshot.db as
        | {
            k6TrAtual?: number;
