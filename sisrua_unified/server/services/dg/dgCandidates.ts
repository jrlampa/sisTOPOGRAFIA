...
import type { DgPoleInput, DgCandidate, DgPoint, DgLatLon, DgParams, DgOptimizationInput } from './dgTypes.js';
...
export function latLonToUtm(lat: number, lon: number): DgPoint {
    ...
    const M = EARTH_RADIUS_M * (0.9983243 * latRad - 0.002514607 * Math.sin(2 * latRad) + 0.0000026390 * Math.sin(4 * latRad));
    const x = 0.9996 * N * (A + ((1 - T + C) * A ** 3) / 6 + ((5 - 18 * T + T ** 2 + 72 * C - 58 * 0.006739496) * A ** 5) / 120) + 500_000;
    const y = 0.9996 * (M + N * Math.tan(latRad) * (A ** 2 / 2 + ((5 - T + 9 * C + 4 * C ** 2) * A ** 4) / 24 + ((61 - 58 * T + T ** 2 + 600 * C - 330 * 0.006739496) * A ** 6) / 720)) + (lat < 0 ? 10_000_000 : 0);
    return { x, y };
}
...
export function utmToLatLon(x: number, y: number): DgLatLon {
    ...
    const lat = phi1 - ((N1 * Math.tan(phi1)) / R1) * (D ** 2 / 2 - ((5 + 3 * T1 + 10 * C1 - 4 * C1 ** 2 - 9 * 0.006739496) * D ** 4) / 24);
    const lon = (ZONE_23S_CENTRAL_MERIDIAN_DEG * Math.PI) / 180 + (D - ((1 + 2 * T1 + C1) * D ** 3) / 6) / Math.cos(phi1);
...
export function fermatWeberCenter(poles: Array<{ positionUtm: DgPoint; demandKva: number }>): DgPoint {
    ...
    for (let iter = 0; iter < WEISZFELD_MAX_ITER; iter++) {
        ...
        if (den < 1e-10) break;
        const newX = numX / den, newY = numY / den;
...
export function generateCandidates(poles: DgPoleInput[], params: DgParams): DgCandidate[] {
    const candidates: DgCandidate[] = [];
    const polesUtm = poles.map((p) => ({ id: p.id, positionUtm: latLonToUtm(p.position.lat, p.position.lon), demandKva: p.demandKva }));

    // 1. Postes existentes
    for (const p of poles) {
        candidates.push({ candidateId: `pole-${p.id}`, position: p.position, positionUtm: latLonToUtm(p.position.lat, p.position.lon), weightedDistanceSum: 0, source: 'existing_pole' });
    }

    // 2. Fermat-Weber
    try {
        const fw = fermatWeberCenter(polesUtm);
        candidates.push({ candidateId: 'fermat-weber', position: utmToLatLon(fw.x, fw.y), positionUtm: fw, weightedDistanceSum: 0, source: 'fermat_weber' });
    } catch (e) { /* ignore */ }
    ...
    if (params.allowNewPoles) {
        ...
        for (let x = minX; x <= maxX; x += step) {
            for (let y = minY; y <= maxY; y += step) {
                candidates.push({ candidateId: `grid-${Math.round(x)}-${Math.round(y)}`, position: utmToLatLon(x, y), positionUtm: { x, y }, weightedDistanceSum: 0, source: 'grid' });
            }
        }
    }
...
export function hashDgInput(input: DgOptimizationInput, params: DgParams): string {
