...
import { BDGD_LAYER_DEFS, type BdgdFieldDef, type BdgdLayerDef } from '../constants/bdgdAneel.js';
...
function validateRecord(
    ...
    for (const field of fields) {
        ...
        if (field.type === 'string' && field.allowedCodes instanceof Set) {
            if (typeof value !== 'string' || !(field.allowedCodes as Set<string>).has(value)) {
                issues.push({
                    ...base,
                    rule: 'R3',
                    severity: 'error',
                    message: `Valor "${value}" inválido para "${field.name}". ` +
        ...
        if (field.type === 'number' && field.allowedCodes instanceof Set) {
            ...
            if (!Number.isFinite(num) || !(field.allowedCodes as Set<number>).has(num)) {
                issues.push({
                    ...base,
                    rule: 'R3',
                    severity: 'error',
...
export function validateBdgdLayer(
    ...
    const validRecords = records.filter((_, idx) =>
        !allIssues.some((issue) => issue.recordIndex === idx && issue.severity === 'error'),
    ).length;
 ...
 */
export function buildBdgdValidationReport(input: BdgdValidationInput): BdgdValidationReport {
