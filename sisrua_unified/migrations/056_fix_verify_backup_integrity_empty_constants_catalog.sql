-- Migration: 056_fix_verify_backup_integrity_empty_constants_catalog.sql
-- Objetivo:
--   Evitar falso-positivo em private.verify_backup_integrity() quando
--   public.constants_catalog está vazio. Nessa situação, backups com row_count=0
--   são esperados e não devem gerar WARNING.
...
BEGIN
  ...
  check_name := 'active_backup_count';
  status := 'ok';
  ...
  SELECT COUNT(*) INTO v_constants_rows
  FROM public.constants_catalog cc
  ...
  IF v_constants_rows = 0 THEN
    status := 'ok';
    detail := 'constants_catalog está vazio; backup não vazio não é esperado';
  ELSE
    status := CASE WHEN v_exists_nonempty THEN 'ok' ELSE 'WARNING' END;
    detail := 'Has any non-empty constants_catalog backup: '
      || CASE WHEN v_exists_nonempty THEN 'yes' ELSE 'no' END;
  END IF;
  RETURN NEXT;
END;
$$;
...
BEGIN
  IF EXISTS (
    ...
    VALUES ('056_fix_verify_backup_integrity_empty_constants_catalog.sql')
