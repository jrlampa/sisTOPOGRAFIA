...
describe('MtRouterPanel component', () => {
  ...
  it('toggles selection modes', () => {
    ...
    const sourceBtn = screen.getByText(/Origem/i).closest('button');
    if (sourceBtn) fireEvent.click(sourceBtn);
    expect(defaultProps.onSetSelectionMode).toHaveBeenCalledWith('picking_source');

    const terminalBtn = screen.getByText(/Terminais/i).closest('button');
    if (terminalBtn) fireEvent.click(terminalBtn);
