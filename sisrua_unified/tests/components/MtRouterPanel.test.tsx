...
describe('MtRouterPanel component', () => {
  ...
  it('toggles selection modes', () => {
    ...
    const sourceBtn = screen.getByRole('button', { name: 'Origem' });
    fireEvent.click(sourceBtn);
    expect(defaultProps.onSetSelectionMode).toHaveBeenCalledWith('picking_source');

    const terminalBtn = screen.getByRole('button', { name: 'Terminais' });
  ...
  it('updates snap distance and CQT params', () => {
    ...
    expect(defaultProps.onSetMtCqtParams).toHaveBeenCalledWith(expect.objectContaining({ voltageKv: 34.5 }));
  });
  ...
  it('renders routing result and allows applying', () => {
    ...
    expect(screen.getAllByText(/5\.00 km/i).length).toBeGreaterThan(0);
  ...
  it('shows error state', () => {
    render(<MtRouterPanel {...defaultProps} state={{ ...mockState, error: 'Engine timeout' } as any} />);
